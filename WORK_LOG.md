## [2026-07-09 13:30] 뉴스 이전/다음(A/N) 단축키 루프의 인덱스 증감 방향 꼬임 교정

**LOG_ID: 20260709_1330**
목표: 뉴스 상세 기사에서 이전글(`A`) / 다음글(`N`) 단축키를 눌렀을 때, 정렬 방향과 인덱스 증감 방향이 꼬여있어 정반대로 기사가 흐르거나 인덱스를 초과해 껑충 뛰어오르는 이동 버그를 최종 해결한다.
변경 파일: `public/js/core/commandRouterService.js`
수행 작업:
1) [진단 및 분석] 
   - BBS 뉴스 피드 배열(`articles`)의 구조는 인덱스가 뒤로 갈수록 기사 번호(`no`)가 커지는 오름차순 방향으로 정렬되어 있었다.
   - 하지만 기존의 단축키 이동 로직은 `A` (이전 기사 = 번호가 더 커지는 방향)를 누르면 `skipIdx = currentIndex - 1` (인덱스 감소)을 돌고, `N` (다음 기사 = 번호가 더 작아지는 방향)을 누르면 `skipIdx = currentIndex + 1` (인덱스 증가)을 도는 방식으로 정반대로 구현되어 있었다. 
   - 이 꼬임 현상으로 인해 `A`를 누르면 기사 번호가 거꾸로 작아지다가 목록 경계를 튕겨 0번째 글(최신 글)로 껑충 점프하는 현상이 발생했음을 팩트 체크를 통해 확인했다.
2) [수정] 
   - `commandRouterService.js` 내 `A` 와 `N` 키 루프의 인덱스 탐색 방향을 올바른 순방향으로 뒤집어 주었다.
   - `A` (이전글) ➡️ `skipIdx = currentIndex + 1` 및 `skipIdx++` (인덱스 증가 방향 = 기사 번호가 더 커지는 방향)
   - `N` (다음글) ➡️ `skipIdx = currentIndex - 1` 및 `skipIdx--` (인덱스 감소 방향 = 기사 번호가 더 작아지는 방향)
3) [검증] `npm test`, `smoke:renderer-ui` 통과 확인.
실행: `npm test`
기대: 단축키 `A`(이전글)를 누르면 껑충 뛰는 일 없이 직전 최신 기사(번호가 커지는 방향)로 차례대로 매끄럽게 연결된다.
결과: ✅ 완료
