## [2026-07-09 16:35] 화면 전환 시 하단 힌트 영역 초기화 보강

**LOG_ID: 20260709_1635**
목표: "본문 전체를 불러올 수 없는 기사입니다..." 같은 일시적인 경고/에러 힌트 메시지가 화면이 기사 목록이나 다른 기사로 전환된 후에도 힌트바에 계속 남아있는 잔상 버그를 수정한다.
변경 파일:
1) `public/js/core/newsScreens.js`
수행 작업:
1) 뉴스 상세 기사(`showNewsArticle`), 뉴스 기사 목록(`showNewsList`), 뉴스 주메뉴(`showNewsMenu`) 등 뉴스 스크린이 전환되는 모든 함수 초입에 `setHint('');`를 추가하여 비동기 데이터 수신이 시작되기 전에 이전 힌트 영역을 즉시 비우도록 개선.
실행: `npm test` 및 문법 체크
기대: 다른 기사로 넘어가거나 목록 화면으로 빠져나가는 즉시 하단의 에러 메시지 힌트 잔상이 사라진다.
결과: ✅ 완료
