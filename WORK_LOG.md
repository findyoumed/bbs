...
목표: "모바일과 pc화면 모두 체크" 5분 주기 확인 루프 — 이번 라운드 회전 화면(이용약관/전체메뉴안내/뉴스 모바일, 대화실/작업기록 데스크톱) 스크린샷 실측.

발견: `/index`(전체 메뉴 안내) 화면을 모바일(390px)에서 스크린샷으로 확인하던 중 하위 메뉴 항목 라벨 다수가 말줄임표 없이 그냥 잘려 다른 단어로 보임을 발견 —
- "전체 메뉴 안내" → "전체 메뉴 안"
- "회원가입 / 로그인" → "회원가입 / 로그"
- "로그인: 이메일" → "로그인: 이메"
- "로그인: 카카오" → "로그인: 카카"
DOM `textContent` 직접 확인으로 실제 데이터 손실(화면 렌더링 문제가 아니라 문자열 자체가 잘려 있음)임을 확인.

원인: `menuIndexScreens.js`의 `buildRow()`가 `fitCell(truncateDisplayText(label, width-1), width)`로 라벨을 절삭 — 이 세션에서 이미 여러 차례(조회수/다운로드수/게시글 제목 등) 발견한 것과 같은 "말줄임표 없는 절삭" 버그 클래스인데, 공용 `fitCellEllipsis` 헬퍼가 있었음에도 이 파일만 놓치고 있었다.

수정: `fitCell(fitCellEllipsis(toLabelText(node), width - 1), width)`로 교체 — 안쪽에서 `width-1` 예산으로 잘릴 때만 "…"를 붙이고, 바깥에서 `width`로 한 번 더 패딩해 코드 칸(GUIDE/LOGIN 등)과 최소 1칸 간격을 보장한다(단순히 `fitCellEllipsis(label, width)`만 쓰면 라벨이 폭을 정확히 채울 때 코드 칸과 공백 없이 붙어버리는 회귀가 생겨 재수정함).

검증: DOM `textContent` 재확인 — 잘리지 않는 라벨은 그대로, 잘리는 라벨은 전부 "…"로 끝나고 코드 칸과 간격 유지 확인. `node --check` 통과, `npm run smoke:menu-wiring` 통과, `npm run smoke:full-traversal` + `node scripts/smoke-mobile-viewports.js` 재통과.

결과: ✅ 버그 수정 완료. 5분 주기 확인 루프 계속 유지.

## [2026-07-27 09:00] [/loop 계속] 27번째 각도: "잘림/넘침" 관련 주석 전수 grep + 명령 힌트바 오버플로우 감지 로직 재검토 — 새 버그 없음, 회귀 스위트 최종 재확인

**LOG_ID: 20260727_0900**
목표: `/loop 다음 수정` 계속 — 코드베이스 전체에서 "잘림/잘렸/넘침/넘쳐/클리핑" 관련 주석이 남은 파일을 재검토하고, 아직 직접 안 본 `terminalHintLayout.js`/`terminalHintFooter.js`(명령 힌트바 줄바꿈 감지·트림 로직)를 점검.

확인:
- `commandService.js`의 관련 주석은 힌트바 우선순위 트림(공간 부족 시 낮은 우선순위 명령부터 숨김)이라는 이미 의도된 동작에 대한 설명일 뿐 버그 아님.
- `listOverflowsLine()`(줄바꿈 감지: 각 항목의 실제 `getBoundingClientRect().top` 비교로 다음 줄로 밀렸는지 판정) / `trimHintEntriesToFit()`(안 밀릴 때까지 낮은 우선순위 항목을 순차적으로 숨김, H 토큰은 항상 유지) 로직을 읽어봄 — 실제 렌더 위치 비교 기반이라 폭 계산 오차에 안전하고, 이번 세션 내내(모든 화면 스모크 테스트) 힌트바 관련 문제가 한 번도 보고되지 않은 것과 일치하는 견고한 설계.
- `grep -rl "잘림\|넘침\|클리핑"`로 나온 16개 파일 전부 이미 이번 세션에 검토했거나(대부분) 의도된 동작 설명 주석으로 확인.

재확인: `npm run smoke:full-traversal`, `node scripts/smoke-mobile-viewports.js`, `npm run smoke:command-parity` 전부 최종 재통과.

결과: ❌ 새 버그 없음 — 6라운드 연속(22~27) 무버그. "잘림/넘침" 키워드로 코드베이스 전체를 훑는 방식까지 소진했다고 판단한다. 다음 라운드는 완전히 다른 유형의 각도가 필요하며, 없다면 사용자에게 계속/중단 여부를 다시 확인해야 한다.

## [2026-07-27 08:45] [/loop 계속] 26번째 각도: 닉네임/채팅 이모지·와이드문자 폭 계산(isWideChar) — 이미 완결된 수정 재확인, 새 버그 없음

**LOG_ID: 20260727_0845**
목표: `/loop 다음 수정` 계속 — 닉네임에 이모지가 섞였을 때 `isWideChar()`의 폭 판정이 실제 렌더 폭과 어긋나 정렬이 깨지거나(오탐이 아니라 진짜 폭 미스매치) `wrapAnsiText`가 잘못된 지점에서 줄바꿈하는지 점검.

확인:
- `isWideChar()`를 직접 읽어보니 U+1F000-1FAFF(이모지 전 대역)이 이미 광폭(2칸) 판정에 포함돼 있고, 실측 근거(LOG_ID 20260726_1615: 동일 이모지 32개로 "32칸" 로 계산됐지만 실제로는 390px 뷰포트에서 614px, 57% 오버플로 — 이미 재현·수정 완료)까지 주석에 남아 있음을 확인.
- 이 세션(더 이전 라운드 포함)에 이미 정확한 실측을 거쳐 완결된 수정이었다 — 오목/오델로 바둑돌 문자(●○◎☎), 박스 문자(─│┌ 등), CJK 전 대역까지 폰트 실측으로 세밀하게 조정된 상태.

결과: ❌ 새 버그 없음(이미 완결된 기존 수정 재확인) — 이번 라운드도 코드 변경 없이 마무리.

**진행 상황 참고**: 22~26라운드(총 5라운드) 연속으로 새로운 버그를 찾지 못했다 — wrapAnsiText/fitCell/CSS 화이트리스트/무제한 리스트 렌더링/숫자 절삭 등 주요 각도는 이미 충분히 소진된 것으로 보인다. 다음 라운드도 계속 새 각도를 찾아보되, 사용자가 중단이나 빈도 조정을 원하면 언제든 반영한다.

## [2026-07-27 08:30] [/loop 계속] 25번째 각도: 검색어 강조(highlightText) 절삭 시 색상 번짐 가능성 — 이론상 결함이나 시각적 영향 없음, 답글 중첩 깊이 재확인

**LOG_ID: 20260727_0830**
목표: `/loop 다음 수정` 계속 — `buildThreadPrefix`(답글 중첩 들여쓰기)와 `highlightText`(검색어 강조) + `fitCell`의 상호작용을 점검.

확인:
- `buildThreadPrefix`는 중첩 깊이를 `min(depth-1, 3)`으로 이미 상한을 두고 있어 아무리 깊게 답글이 달려도 들여쓰기가 무한정 늘어나지 않음 — 안전.
- `highlightText`로 강조된(ANSI 색상 코드로 감싼) 제목이 `fitCell`에 의해 폭 중간에서 잘리면, 이론상 강조 색상을 닫는 리셋 코드가 잘려나가 이후 문자에 색이 "번질" 수 있다는 가설을 세우고 `fitCell` 로직을 추적 — 실제로 그런 경로가 존재함을 코드로 확인했다.
- 그런데 이 세션 내내 확인한 모든 목록 줄(`ansiBoardBuilders.js`)이 줄 끝에 예외 없이 명시적 `ANSI_RESET`을 한 번 더 붙이고 있어(`line += ... + ANSI_RESET`), 색상 번짐이 있어도 그 줄이 끝나기 전에 다시 리셋된다. 게다가 번짐이 미치는 범위는 fitCell의 트레일링 패딩 공백뿐인데, 공백 문자는 전경색이 달라져도 눈에 보이는 잉크가 없어 시각적으로 전혀 구분되지 않는다 — 코드 상으로는 존재하는 결함이지만 실사용자에게 보이는 영향이 0이라 CLAUDE.md의 "실제 영향 없는 것 방어 금지" 원칙에 따라 수정하지 않는다.

결과: ❌ 새 버그 없음(시각적 영향 없음으로 판정) — 이번 라운드도 코드 변경 없이 마무리. 다음 라운드는 26번째 각도를 탐색한다.
...
목표: `/loop 다음 수정` — 계속되는 UI 잘림 전수조사에서 아직 안 본 초기화면(main)의 "작은공지" 배너 렌더링 로직을 점검.

발견:
- `ansiBoardBuilders.js`의 `buildMainMenuAnsi`에서 `noticeText`(작은공지)를 렌더할 때 `fitCell(titleOnly, remaining).trim()`을 쓰는데, 이는 이번 세션 내내 찾아 고쳐온 "말줄임표 없는 fitCell 절삭"과 똑같은 결함이다 — 공지 문구가 길면 안내 없이 그냥 잘린다.
- 그런데 실제 호출부(`menuNavigation.js:156`)를 확인하니 `const noticeText = null;`로 하드코딩되어 있고, 코드베이스 전체에서 `buildMainMenuAnsi`를 부르는 곳이 이 한 곳뿐이다 — 즉 이 배너 기능은 실제로 살아있는 데이터 경로가 전혀 없는 죽은 코드다(나우누리 테마 분기도 동일하게 `noticeText`를 그대로 전달만 함).

결과: ❌ 실사용 버그 아님(죽은 코드) — CLAUDE.md의 "일어날 수 없는 시나리오에 방어 코드 추가 금지" 원칙에 따라 수정하지 않는다. 다음 라운드는 24번째 각도를 탐색한다.

## [2026-07-27 07:45] [/loop 계속] 22번째 각도: @media print/테마 충돌/fitCountCell 경계값 점검 — 새 버그 없음

**LOG_ID: 20260727_0745**
목표: `/loop 다른 메뉴의 ui는 글자잘림없는지 전수조사 실행해` 계속 — 남은 가능성을 폭넓게 점검.

확인:
- `@media print` 규칙 자체가 style.css/styles/*.css 어디에도 없음 — "PC통신 갈무리" 기능은 브라우저 인쇄가 아니라 앱 내부 시뮬레이션(`data-print-view`)이라 무관함을 재확인.
- `body.theme-blue` 등 테마 규칙은 색상만 바꾸고 `overflow`/`min-height` 속성을 전혀 건드리지 않아, 이번 세션에 추가한 화이트리스트들과 충돌 위험 없음.
- style.css 중괄호 짝(466/466)과 `data-screen` 중복 등장 횟수 확인 — 구조적 이상 없음.
- 직전 라운드에 만든 `fitCountCell` 헬퍼의 경계값(width=0, width=1, 음수 입력)을 직접 테스트 — 전부 합리적으로 처리됨(width=1일 때 두 자리 이상 값은 그냥 `"+"`만 표시, 음수는 카운트 필드에 실제로 발생하지 않아 무관).

결과: ❌ 새 버그 없음 — 이번 라운드도 코드 변경 없이 마무리. 다음 라운드는 23번째 각도를 탐색한다.

## [2026-07-27 07:30] [/loop 계속] 21번째 각도: 실사용 데이터 기반 시각 확인(스크린샷) — 새 버그 없음, 세션 전체 요약

**LOG_ID: 20260727_0730**
목표: `/loop 다른 메뉴의 ui는 글자잘림없는지 전수조사 실행해` 계속 — 지금까지는 코드 분석과 DOM 수치 측정 위주였는데, 이번엔 접근을 바꿔 실제 dev 서버의 진짜 데이터로 5개 주요 화면(초기화면/게시판 목록/설문 목록/대화실 대기실/도움말)을 390x844 뷰포트에서 직접 스크린샷 찍어 육안 확인.
- 추가로 조회수 절삭 관련 코드(`fitCell(String(vote.totalVotes...), 8)`, `Math.round((count/totalVotes)*100)`) 등 나머지 숫자 표시 지점도 훑어봄 — division-by-zero 가드 있음, 투표수 같은 카운터는 실질적으로 6자리를 넘길 가능성이 극히 낮아 우선순위 낮음으로 판단.

확인: 5개 스크린샷 전부 잘림·정렬 깨짐 없이 깨끗하게 렌더됨(테스트 게시글 제목에 ISO 타임스탬프가 그대로 들어간 것은 스모크 테스트 잔재 데이터일 뿐 렌더링 버그 아님).

결과: ❌ 새 버그 없음.

**이번 /loop 세션 누적 요약** (약 21라운드): 실제로 찾아 고친 버그 14건 — ①접속자 목록 무제한 렌더 ②궁합(compat-result) 페이지네이션 부재 ③min-height:1.32em 화이트리스트 최초 7개 화면 ④PDS 첨부파일 목록 무제한 렌더 ⑤설문 선택지 무제한 렌더(페이지네이션으로 해결) ⑥명령 이력 무제한 렌더 ⑦대화실 대기실 접속자/방 목록 무제한 렌더 ⑧토론의 광장 회의실/안건 목록 무제한 렌더 ⑨쪽지함 무제한 렌더 ⑩PT 명령 설계 결함(100건 출력이 25행 격자를 넘음, 페이지네이션으로 해결) ⑪⑫⑬CSS 화이트리스트 누락 3라운드(총 21개 화면) ⑭숫자 칸(조회수 등) 절삭이 다른 값처럼 보이는 문제. 다음 라운드는 22번째 각도를 탐색하거나, 사용자가 중단을 원하면 그때 멈춘다.

## [2026-07-27 07:15] [/loop 계속] 20번째 각도: 조회수/다운로드수/안건수 등 숫자 칸 절삭 — 값이 다른(더 작은) 숫자로 보이는 버그 발견 및 수정

**LOG_ID: 20260727_0715**
목표: `/loop 다른 메뉴의 ui는 글자잘림없는지 전수조사 실행해` 계속 — 지금까지는 전부 "리스트 줄 수"·"세로 클리핑" 관점이었는데, 완전히 새로운 각도로 "숫자를 담는 고정폭 칸(fitCell)"을 점검.

발견:
- `fitCell(text, maxWidth, align)`은 `align`과 무관하게 항상 왼쪽부터 문자를 채우다 폭을 넘기면 그 자리에서 멈춘다 — 텍스트에는 이미 `fitCellEllipsis`(말줄임표)로 여러 곳을 고쳐뒀지만, 조회수/다운로드수/안건수/재청수 같은 "숫자" 칸은 그냥 `fitCell`을 썼다.
- 직접 테스트로 확인: `fitCell('12345', 4, 'right')` → `"1234"`. 이건 텍스트 절삭과 성격이 다르다 — 잘렸다는 티가 전혀 안 나고, 그냥 **실제보다 작은, 그러나 완전히 그럴듯한 다른 숫자**로 보인다(조회수 12345회를 1234회로 오인). 텍스트는 "잘린 것 같다"는 느낌이라도 주지만 숫자는 그 자체로 완결된 값처럼 보여 더 위험한 변종이었다.
- 영향 범위: 게시판 목록(조회수, `ansiBoardBuilders.js`/`postListView.js`), PDS 자료실 목록(다운로드수), 토론의 광장 회의실 목록(안건수)/안건 목록(재청수) — 전부 조회/다운로드/추천처럼 시간이 지나며 자연히 계속 증가하는 값이라 장기 운영 시 실제로 도달 가능한 자릿수.

구현:
- `ansiBuilderUtils.js`에 `fitCountCell(value, maxWidth, align='right')` 헬퍼 추가 — 값이 칸에 들어가면 기존과 동일하게 표시하고, 넘칠 때만 `'9'.repeat(maxWidth-1) + '+'`(예: 폭 4 → `"999+"`)로 바꿔 "적어도 이보다 크다"는 것만은 알 수 있게 한다. ID(게시글 번호/방 번호/투표 번호)처럼 사용자가 실제로 입력해 선택하는 값은 그대로 두었다 — 그런 값에 "999+"를 보여주면 오히려 조작이 불가능해지므로, 순수 표시용 카운터에만 적용했다.
- `ansiBoardBuilders.js`(조회수·다운로드수 2곳), `postListView.js`(PT 조회수), `confAnsiBuilders.js`(안건수·재청수 2곳) — 총 5곳을 `fitCountCell`로 교체.

검증:
- `node --check`로 4개 수정 파일 모두 문법 통과.
- 직접 함수 호출 재현 테스트: `fitCountCell(9999,4)`→`"9999"`(경계, 정상 표시), `fitCountCell(12345,4)`→`"999+"`, `fitCountCell(123456,5)`→`"9999+"` 확인. `buildPostListAnsi`/`buildConfRoomListAnsi` 전체 렌더로도 칸 정렬이 그대로 유지됨을 확인(폭이 항상 고정 유지).
- `npm run smoke:full-traversal`, `node scripts/smoke-mobile-viewports.js`(3개 뷰포트 × 27개 라우트), `npm run smoke:command-parity` 모두 통과.

결과: ✅ 완료 — 이번 세션 내내 "줄 수/세로 공간" 관점의 버그만 찾다가, 처음으로 "숫자 칸 절삭"이라는 다른 성격의 버그를 발견해 고쳤다. 다음 라운드는 21번째 각도를 탐색해야 한다.
...
목표: `/loop 다른 메뉴의 ui는 글자잘림없는지 전수조사 실행해` 계속 — 직전 라운드에서 보류한 5개 화면 중 방문 빈도가 가장 높은 post-list부터 근본 원인을 정밀 조사.

조사 과정:
- 360×400에서 `/board/plaza`의 `#terminal-screen` DOM을 직접 뜯어봄 — 본문(`.ansi-screen-body`)은 게시글 15개 + 컬럼헤더 + 구분선 = 17줄로 예상대로였고 특별히 길지 않았다.
- 그런데 `scrollHeight(473px) - clientHeight(310px) = 163px` 초과분이 나왔는데, 17줄 분량치고는 지나치게 크다 — 상단바(`retro-topbar--ansi`, 로고/실시간 시계/구분선)도 `#terminal-screen`의 자식이라 `scrollHeight` 계산에 함께 들어가는데, 상단바 자체의 패딩/여백이 지금까지 이 세션 내내 "4줄"로 가정해온 것보다 실제로 더 많은 공간을 차지하고 있을 가능성이 높다.
- 즉 이 초과분은 게시글 목록의 줄 수 문제(policy처럼 `linesPerPage`를 줄이면 되는 문제)가 아니라, **모든 화면이 공유하는 상단바/프레임 레이아웃 자체**의 문제일 수 있다는 결론에 도달했다 — 이건 개별 화면 하나만 손보는 것보다 훨씬 파급력이 크고 위험한 변경(전체 화면에 영향)이라, 충분한 사전 검증 없이 이번 라운드 안에서 서둘러 고치지 않기로 판단했다.
- post-list는 이 프로젝트의 공식 스모크 뷰포트(320x568 이상)에서는 여전히 안전(직전 라운드 확인)하고, 360x400은 이 세션에서도 "극단적으로 짧은 뷰포트" 취급을 받아온 값이라, 실사용 영향은 제한적으로 판단된다.

결과: ⏸️ 코드 수정 보류 — 근본 원인이 개별 화면이 아니라 상단바/프레임 공용 레이아웃일 가능성이 높아, 리스크가 큰 전역 변경을 성급히 하지 않고 다음 라운드(또는 사용자 확인 후)에 상단바 실제 렌더 높이를 별도로 정밀 측정하는 것부터 시작하기로 한다. 다음 라운드는 상단바 정밀 측정을 이어가거나 18번째 각도를 새로 탐색한다.

## [2026-07-27 06:00] [/loop 계속] 17번째 각도: 나머지 미등록 화면 5개 재검사 — 확인했지만 이번 라운드엔 수정 보류(설계상 이유)

**LOG_ID: 20260727_0600**
목표: `/loop 다른 메뉴의 ui는 글자잘림없는지 전수조사 실행해` 계속 — 직전 라운드(8개 화면 화이트리스트 추가)가 적절한 범위였는지 검증하고, 여전히 화이트리스트에 없는 나머지 화면(main/post-list/vote-list/weather-menu/menu-index)도 같은 문제가 있는지 확인.

확인:
- 먼저 이 프로젝트의 공식 스모크 테스트 뷰포트(smoke-mobile-viewports.js의 390x844/360x740/320x568, 가장 짧은 게 320x568)로 재검사 — main/post-list/vote-list/weather-menu/menu-index 전부 클리핑 없음(post-list만 320x568에서 3px, 오차 수준). 즉 이 프로젝트가 공식적으로 검증하는 실기기 범위에서는 문제가 없다.
- 그런데 이 세션에서 policy/my-stats를 고칠 때 실제로 썼던 더 가혹한 기준 뷰포트(360x400)로 재검사하니 이 5개 화면도 전부 클리핑됨(main 19px, post-list 163px, vote-list 43px, weather-menu 67px, menu-index 115px)을 확인.
- 하지만 이 5개는 직전 라운드에 고친 8개(history/chat-lobby 등, "한 번 보고 끝"인 목록)와 성격이 다르다 — post-list/vote-list/weather-menu는 이미 F/B 페이지네이션으로 이동하는 화면이고, news-view 관련 기존 주석(20260707_1528)이 명시하듯 "PC통신은 페이지 단위로 이동하지, #terminal-screen 자체가 스크롤 영역이 되면 스크롤바가 생겨 터미널 UI가 깨진다"는 게 이 프로젝트의 의도된 설계 원칙이다. 이 화면들의 올바른 해법은 스크롤 허용이 아니라 policy가 이미 겪은 것처럼 `linesPerPage`를 줄이는 화면별 미세조정이며, main도 마찬가지로 항목 수/여백을 조정하는 게 맞다 — 8개 화면에 썼던 것과 같은 CSS 화이트리스트 일괄 추가를 그대로 적용하면 이미 페이지네이션이 완비된 화면에 불필요한 스크롤바가 생겨 설계 일관성이 깨질 위험이 있다.

결과: ⏸️ 이번 라운드엔 코드 수정 보류 — 5개 화면 모두 이 프로젝트의 공식 테스트 범위(320x568 이상)에서는 안전하며, 더 가혹한 360x400에서만 미세한 클리핑이 있다. 성급하게 화이트리스트를 일괄 적용하기보다 각 화면의 `linesPerPage`/여백을 개별적으로 재검토하는 게 맞는 해법이라 판단해, 다음 라운드에 시간을 들여 화면별로 신중하게 접근하기로 하고 이번엔 위험한 일괄 변경을 피했다. 다음 라운드는 이 5개 화면 중 하나(우선순위: 방문 빈도가 가장 높은 post-list)의 정밀한 linesPerPage 재조정부터 시작하거나, 18번째 각도를 새로 탐색한다.
...
목표: `/loop 다른 메뉴의 ui는 글자잘림없는지 전수조사 실행해` 계속 — 13번째 각도로 `postListView.js`의 남은 `.slice(0, 100)` 지점을 확인.

발견:
- `showPtResult`(PT 명령, "100건 제목 일괄 출력" 기능)가 최대 100개 게시글 제목을 `filtered.forEach`로 전부 렌더 → `renderAnsiScreenWithTopbar`를 거쳐 ansiEngine.js의 25행 고정 격자(ROWS=25, `putChar`가 `row>=25`면 HTML로 렌더되기도 전에 문자를 그냥 버림)에 그대로 들어간다. `pt-view` 화면은 style.css의 어떤 `overflow-y:auto` 화이트리스트에도 없어, 20개만 넘어도 나머지가 100% 유실됐다.
- 이번 세션에 찾은 다른 버그들과 달리 이건 "예외적으로 많을 때만 생기는 결함"이 아니라, 기능 자체의 존재 목적(제목 100건을 한 번에 출력)이 그 화면이 물리적으로 담을 수 있는 양(약 20건)을 애초부터 넘는, 가장 직접적인 형태의 설계 결함이었다.

구현:
- 단순 절삭+안내로는 "100건 일괄 출력"이라는 기능 목적 자체가 무의미해지므로, 게시글 보기/설문 상세와 동일하게 페이지네이션을 적용(`PT_LINES_PER_PAGE=16`).
- 페이지 이동 시 서버 재조회를 피하려고 첫 조회 결과를 `state._ptFilteredPosts`에 캐시(2쪽 이상 요청 시 캐시 재사용), 새 PT 실행(`showPtPrepare`)마다 캐시를 지워 이전 실행 결과가 새 시작 번호에 섞이지 않게 함.
- `commandRouterBrowse.js`의 `pt-view` 블록에 F키 다음 페이지 처리 추가(기존엔 아무 키나 누르면 무조건 목록으로 돌아갔는데, F는 다음 페이지로 넘기고 그 외 키만 목록으로 복귀 — 마지막 페이지에서는 F도 목록으로 감).

검증:
- `node --check`로 두 수정 파일 모두 문법 통과.
- `createPostListView`를 직접 호출하는 Node 테스트(전역 `window` 스텁 + `apiFetch` 스텁으로 실제 함수 로직 그대로 실행): 5/16건은 1쪽(20줄 이내)으로 전부 표시, 17/30/100건은 각각 2/2/7쪽으로 정확히 분할되고 각 쪽이 20줄 이내(수정 전엔 30건만 돼도 30줄 그대로 렌더돼 5줄이 25행 격자 밖으로 유실되던 것과 대비).
- `npm run smoke:full-traversal`, `node scripts/smoke-mobile-viewports.js`(3개 뷰포트 × 27개 라우트), `npm run smoke:command-parity` 모두 통과.

결과: ✅ 완료 — 접속자 목록/첨부파일 목록/설문 선택지/명령 이력/대화실 대기실/토론의 광장/쪽지함에 이어 여덟 번째 "안내 없는 절삭" 버그지만, 유일하게 "단순 안내로는 기능이 무의미해지는" 케이스라 페이지네이션으로 다르게 접근했다. 다음 라운드는 14번째 각도를 탐색해야 한다.

## [2026-07-27 04:45] [/loop 계속] 쪽지함(memo list) — 16통 넘으면 안내 없이 잘림 발견 및 수정

**LOG_ID: 20260727_0445**
목표: `/loop 다른 메뉴의 ui는 글자잘림없는지 전수조사 실행해` 계속 — 12번째 각도로 `.slice(0, N)` 재점검 방식을 이어가, 아직 안 본 `memoAnsiBuilders.js`(쪽지 목록)와 `newsScreens.js`(뉴스 핫스팟)를 확인.

발견:
- `memoAnsiBuilders.js`의 `buildMemoListAnsi(memos, ...)`가 쪽지함(받은/보낸/보관) 목록을 16통으로 자르는데 초과 안내가 없었다. 헤더의 "(총 N통)"은 진짜 총량을 그대로 보여주면서 정작 목록엔 왜 그보다 적게 보이는지 안내가 전혀 없는 앞뒤 불일치.
- `memoScreens.js`가 `/api/memos?box=...`를 페이지 파라미터 없이 통째로 받아오고, 쪽지는 다른 사람이 보내오는 대로 자연히 쌓이는 데이터라(회의실/설문처럼 의도적으로 많이 만들어야 하는 게 아니라 평범한 사용만으로도 16통을 넘기기 쉬움) 이번 세션에 찾은 것 중 가장 재현되기 쉬운 케이스.
- `newsScreens.js`의 `articles.slice(0, 15)`(뉴스 핫스팟 부착)는 텍스트 절삭이 아니라 이미 15개 단위로 페이지네이션된 뉴스 목록(`NEWS_LIST_PAGE_SIZE=15`, 실측 확인)에 맞춰 클릭 영역을 붙이는 로직이라 안전함을 확인.

구현:
- `buildMemoListAnsi`는 끝에서 24줄까지 빈 줄만 채우고 잘라내지 않는(패딩 전용) 구조임을 먼저 확인해 안내 줄 추가가 항상 안전함을 검증한 뒤, `... 외 N통 더 있습니다.` 안내를 추가.

검증:
- `node --check memoAnsiBuilders.js` 통과.
- 직접 함수 호출 테스트: 5/16통(경계, 초과 없음)은 안내 없이 전부 표시, 17/30통은 16통 표시 + 안내, 모든 경우 총 줄 수 24줄 유지(패딩 로직이 흡수).
- `npm run smoke:full-traversal`, `node scripts/smoke-mobile-viewports.js`(3개 뷰포트 × 27개 라우트), `npm run smoke:command-parity` 모두 통과.

결과: ✅ 완료 — 접속자 목록/첨부파일 목록/설문 선택지/명령 이력/대화실 대기실/토론의 광장에 이어 일곱 번째로 발견한 "안내 없는 절삭" 버그. 다음 라운드는 13번째 각도를 탐색해야 한다.

## [2026-07-27 04:30] [/loop 계속] 토론의 광장(FORUM) 회의실 목록/안건 목록 — 안내 없는 절삭 발견 및 수정

**LOG_ID: 20260727_0430**
목표: `/loop 다른 메뉴의 ui는 글자잘림없는지 전수조사 실행해` 계속 — 11번째 각도로, 직전 라운드(chat-lobby)에서 효과를 본 "`.slice(0, N)` 재점검" 방식을 코드베이스 전체 grep 결과에 계속 적용.

발견:
- `confAnsiBuilders.js`의 `buildConfRoomListAnsi(rooms)`/`buildConfAgendaListAnsi(room, agendas)`가 각각 회의실 목록·안건 목록을 12개로 자르는데 초과 안내가 전혀 없음.
- 서버(`ConfRepositoryMemory.js`/`ConfRepositorySupabase.js`의 `createRoom`/`createAgenda`)에 개수 상한이 전혀 없고, 클라이언트도 `apiFetch('/api/conf/rooms?closed=1')`/`.../agendas`를 페이지 파라미터 없이 통째로 받아오므로, 로그인한 사용자가 회의실이나 안건을 13개 넘게 만들기만 해도 나머지가 안내 없이 사라짐(설문 선택지·투표 관련 버그와 동일하게 상한 없는 사용자 생성 데이터).
- 회의실/안건 진입은 번호를 직접 입력해 이동하므로(`commandRouterConf.js`) 목록에 안 보여도 기능적으로 막히진 않지만, 목록에 없으면 그 번호가 존재하는지 자체를 알 방법이 없다는 점에서 접속자 목록/첨부파일 목록과 동일한 발견 가능성 문제.

구현:
- 이 두 함수는 (chat-lobby와 달리) 끝에서 강제로 줄 수를 자르는 로직이 없어(19줄 안팎으로 이미 여유가 있음을 직접 계산·확인) 항목 하나를 안내 줄로 교체할 필요 없이, 그냥 안내 줄을 추가만 해도 안전함을 먼저 확인한 뒤 단순하게 `... 외 N개 회의실/안건 더 있습니다.` 안내를 추가.

검증:
- `node --check confAnsiBuilders.js` 통과.
- 직접 함수 호출 테스트: 회의실/안건 각각 5/12개(경계, 초과 없음)는 안내 없이 전부 표시, 13/20개는 12개 표시 + 안내. 최악 케이스(20개)에서 총 줄 수가 회의실 20줄·안건 20줄로 25행 예산 안에 여유 있게 들어감을 확인(직전 라운드에서 겪은 "안내 줄 추가로 예산 초과" 실수가 여기선 재발하지 않음을 사전에 계산으로 확인 후 적용).
- `npm run smoke:full-traversal`, `node scripts/smoke-mobile-viewports.js`(3개 뷰포트 × 27개 라우트), `npm run smoke:command-parity` 모두 통과.

결과: ✅ 완료 — 접속자 목록/첨부파일 목록/설문 선택지/명령 이력/대화실 대기실에 이어 여섯 번째로 발견한 "안내 없는 절삭" 버그. `.slice(0, N)` grep 재점검 방식이 이번에도 유효했다. 다음 라운드는 12번째 각도를 탐색해야 한다.

## [2026-07-27 04:15] [/loop 계속] 대화실 대기실(chat-lobby) 접속자/방 목록 — 안내 없는 절삭 발견 및 수정 (+ 내 실수도 함께 고침)

**LOG_ID: 20260727_0415**
목표: `/loop 다른 메뉴의 ui는 글자잘림없는지 전수조사 실행해` 계속 — 10번째 각도(검색/힌트바) 소진 후, `.slice(0, N)` 패턴을 코드베이스 전체에서 다시 grep해 "안내 문구 짝이 맞는지" 재검토.

발견:
- `chatAnsiBuilders.js`의 `buildChatLobbyAnsi`에서 방 목록/참여자 미리보기는 "외 N명"/"외 N개"로 초과분을 안내하는데, 바로 위 대기실 접속자 목록(`maxUsersToShow`, 데스크톱 6명/모바일 4명)과 방 목록 자체(`maxRoomsToShow`, 4개/3개, 개설방 최대 100개까지 가능)는 안내 없이 조용히 잘렸다 — 같은 화면 안에서 참여자 미리보기만 고쳐지고 두 상위 목록은 빠졌던 불일치.
- 직접 함수 호출 테스트로 확인: 접속자 7명 이상이면 6명만 보이고 안내 없음, 방 5개 이상이면 4개만 보이고 안내 없음.

구현 및 자체 실수 수정:
- 처음엔 단순히 "넘치면 안내 줄 추가"로 고쳤는데, 이 화면은 맨 끝에서 `joinedLines.slice(0,24)`로 24줄 고정 캔버스에 맞춰 자르는 구조라 접속자 안내 줄 + 방 안내 줄을 둘 다 무조건 추가하면 총 25줄이 되어 정작 방 안내 줄 자신이 최종 절삭에 밀려나는 걸 직접 테스트로 발견(이번 세션에 반복 경고해온 "예산 초과분을 다시 초과시키는" 실수를 스스로도 저지를 뻔함).
- 접속자 목록/방 목록 모두 "넘칠 때는 마지막 항목 하나를 안내 줄로 교체"하는 방식으로 재수정 — 이 섹션이 쓰는 줄 수 자체가 넘침 여부와 무관하게 항상 고정되도록(방은 참여자 유무에 따라 1~2줄이라 항목 하나를 빼면 최소 1줄은 항상 확보됨).

검증:
- `node --check chatAnsiBuilders.js` 통과.
- 직접 함수 호출 재현 테스트: 접속자 3/6명(경계, 초과 없음)은 안내 없이 전부 표시, 7/10/15명은 5명 표시 + 안내; 방 목록도 접속자 15명+방 10개(참여자 꽉 채운 최악 케이스)에서 총 줄 수가 정확히 24줄로 유지되며 두 안내 줄이 모두 살아있음을 확인(수정 전 시도에서는 25줄이 되어 방 안내 줄이 사라졌었음). 정확히 경계값(6명/4개 방, 초과 없음)에서도 24줄 유지 확인.
- `npm run smoke:full-traversal`, `node scripts/smoke-mobile-viewports.js`(3개 뷰포트 × 27개 라우트), `npm run smoke:command-parity` 모두 통과.

결과: ✅ 완료 — 접속자 목록/첨부파일 목록/설문 선택지/명령 이력에 이어 다섯 번째로 발견한 "안내 없는 절삭" 버그. 동시에 고정 줄수 예산 안에서 여러 안내 줄을 추가할 때는 전체 예산을 재계산해야 한다는 교훈을 스스로 실측으로 재확인했다. 다음 라운드는 11번째 각도를 탐색해야 한다.

## [2026-07-27 04:00] [/loop 계속] 10번째 각도: 게시판 검색/주제어 목록/힌트바·알림 점검 — 새 버그 없음

**LOG_ID: 20260727_0400**
목표: `/loop 다른 메뉴의 ui는 글자잘림없는지 전수조사 실행해` 계속 — 9번째 각도(글쓰기/회원가입) 소진 후, 게시판 검색 기능(LT/GL/SUBJ/LI/GA/BODY)과 주제어(KW) 집계, 힌트바/알림 토스트를 점검.

확인:
- `KW`(주제어목록) 명령(`commandRouterBrowse.js`)은 게시판 전체 글의 주제어를 모아 쉼표로 이어붙인 무제한 길이 문자열을 만들지만, ANSI 격자가 아니라 `setHint()`로 힌트바(`#cmd-hint`)에 표시된다 — 이 힌트바는 이미 이전 세션에 `text-overflow:ellipsis`로 고쳐져 있어(LOG_ID 20260726_0945, 정확히 이 KW 사례를 언급하며 고친 전례) 잘리더라도 "잘렸다"는 것을 사용자가 알 수 있다. 추가 조치 불필요.
- 직접 검색 명령(`LT/GL/SUBJ/LI/GA/BODY [검색어]`)은 별도의 결과 목록 빌더 없이 기존 `showPostList`(이미 페이지네이션 적용된 게시글 목록 함수)를 그대로 재사용하므로 안전.
- 알림 토스트(`.terminal-notification-row`)는 `white-space: pre-wrap`이라 자연스럽게 줄바꿈되며 잘리지 않음.
- 채팅에는 멘션/자동완성 드롭다운 같은 별도 기능 자체가 없어 해당 없음.

결과: ❌ 새 버그 없음 — 이번 라운드도 코드 변경 없이 마무리. 10개 각도(wrapAnsiText/fitCell/CSS whitelist×2/raw-HTML 트랜스크립트/무제한 리스트 렌더링/랜드스케이프(오탐)/화면모듈 내부 리스트 절삭/정적 메뉴 목록/글쓰기·회원가입/검색·힌트바)를 모두 소진했다. 이번 세션에서 반복 발견된 버그 클래스(안내 없는 무제한 리스트 절삭)에 해당하는 화면은 이제 대부분 찾아 고친 것으로 보이며, 다음 라운드는 지금까지와 전혀 다른 새로운 유형의 각도(예: 다크/라이트 테마 전환 시 렌더링, 폰트 로딩 실패 폴백, 키보드 표시/숨김 전환 타이밍)를 찾아야 한다.

## [2026-07-27 03:45] [/loop 계속] 9번째 각도: 게시글 작성/수정 에디터·회원가입 화면 점검 — 새 버그 없음

**LOG_ID: 20260727_0345**
목표: `/loop 다른 메뉴의 ui는 글자잘림없는지 전수조사 실행해` 계속 — 8번째 각도(정적 메뉴 목록) 소진 후, 지금까지 ANSI 격자 화면 위주로만 조사해온 것을 벗어나 게시글 작성/수정(postWriteView.js)과 회원가입(signup*.js) 화면을 처음으로 점검.

확인:
- 현재 실제로 쓰이는 글쓰기 경로(`renderBbsEditor`, "박스 에디터")는 본문을 진짜 `<textarea>`로 렌더한다 — ANSI 25행 고정 격자(ansiEngine.js)를 전혀 거치지 않고 브라우저 네이티브 스크롤을 그대로 쓰므로, 아무리 길게 써도 텍스트가 유실될 수 없는 구조.
- 옛 줄 단위 에디터(`renderLineEditor`, "/L 목록" 등)는 코드는 남아있지만 주석(20260726_1745)에 따르면 "bbs-form이 항상 먼저 걸려 도달 불가능한 죽은 코드"다. 그런데도 이미 `MAX_VISIBLE_TRANSCRIPT_LINES=18` + "(... 이전 N줄 생략 ...)" 안내 문구(LOG_ID 20260710_1640)가 적용돼 있어, 혹시라도 재사용되더라도 안전.
- 회원가입 화면(signupScreens.js/signupFlowUi.js)의 이메일 가입 트랜스크립트·약관 동의 박스는 전부 고정된 소수의 단계/정적 약관 텍스트(스크롤 가능한 `.entry-signup-agreement-box`)로, 이전 라운드(raw-HTML 트랜스크립트 점검, LOG_ID 20260727_0200)에서 이미 "짧고 고정된 값이라 위험 없음"으로 판정한 것과 동일한 범주.

결과: ❌ 새 버그 없음 — 이번 라운드도 코드 변경 없이 마무리. 9번째 각도(글쓰기/회원가입 흐름)를 소진했다고 판단하며, 남은 화면 표면 중 아직 안 본 것(대화방 메시지 입력 자동완성, 검색 결과 화면 등)을 다음 라운드에서 이어서 찾아본다.

## [2026-07-27 03:30] [/loop 계속] 8번째 각도: 게시판 메뉴(board-select)/전체 메뉴 안내 점검 — 새 버그 없음

**LOG_ID: 20260727_0330**
목표: `/loop 다른 메뉴의 ui는 글자잘림없는지 전수조사 실행해` 계속 — 7번째 각도(화면 모듈 내부의 안내 없는 리스트 절삭)에서 `/history`를 찾은 뒤, 같은 각도로 나머지 화면들을 재확인.

확인:
- `ansiBoardBuilders.js`의 `buildBoardSelectAnsi(boards, ...)`가 `boards.forEach`로 게시판 목록을 슬라이스/절삭 전혀 없이 전부 렌더하는 것을 발견 — 접속자 목록/첨부파일 목록/설문 선택지/명령 이력과 같은 잠재적 위험 패턴으로 보여 조사.
- 다만 이 목록은 `legacy/hanulso.mnu`(정적 메뉴 설정 파일)에서 오는 것이라 사용자 데이터처럼 무한정 자라지 않는다 — XML을 실제로 파싱해 카테고리별 하위 항목 수를 세어보니 가장 큰 카테고리(오락실/GAME)도 17개, 게시판(BBS)은 10개뿐이었다. 헤더 4줄 + 17개 항목 = 21줄로 25행 예산 안에 여유 있게 들어가 실제로 잘릴 데이터가 없다 — activity-summary의 `limit=5` 사례처럼 "코드 경로는 무제한이지만 실사용 데이터는 절대 그 임계값에 닿지 않는" 경우로 판단, 방어적 코드 추가는 보류(CLAUDE.md의 "일어날 수 없는 시나리오에 대한 처리 추가 금지" 원칙).
- `menuIndexScreens.js`의 `buildMenuIndexAnsi`(전체 메뉴 안내, 메뉴 트리 전체를 보여주는 화면이라 이론상 가장 큰 목록)는 이미 20260725_0830에 `linesPerPage=15` 페이지네이션이 적용돼 있어 안전함을 재확인.
- 버디 목록/추천인 목록처럼 별도의 "무제한 사용자 목록 화면"이 될 만한 기능은 코드베이스에 존재하지 않음(채팅 버디는 접속자 목록에서 강조 표시로만 쓰이고 별도 목록 화면이 없음).

결과: ❌ 새 버그 없음 — 이번 라운드는 실제 코드 변경 없이 마무리. 8번째 각도(정적/동적 메뉴 목록 렌더링)를 소진했다고 판단하며, 다음 라운드는 9번째 각도(예: 게시글 작성/수정 미리보기, 회원가입 흐름 화면)를 탐색해야 한다.

## [2026-07-27 03:15] [/loop 계속] 명령 이력(/history) 화면 — 20개 넘게 쌓이면 안내 없이 조용히 잘림 발견 및 수정

**LOG_ID: 20260727_0315**
목표: `/loop 다른 메뉴의 ui는 글자잘림없는지 전수조사 실행해` 계속 — 6번째 각도(랜드스케이프)가 오탐으로 끝난 뒤, 7번째 각도로 `helpScreens.js`/`menuIndexScreens.js`/`policyScreens.js` 등 별도 `*AnsiBuilders.js` 파일이 아닌 화면 모듈 내부에 직접 박혀 있는 forEach/map 렌더링 로직을 재검토.

발견:
- `helpScreens.js`의 `buildHistoryAnsi()`가 `state.cmdHistory` 전체를 한 줄씩 렌더한 뒤, 함수 맨 끝에서 `joinedLines.slice(0, 24)`로 24행 고정 캔버스에 맞춰 잘랐다 — 안내 문구 없이 조용히 잘림.
- `state.cmdHistory`는 `appEventsCommandInput.js`(`unshift` 후 `length>50`이면 `pop`)에 의해 최대 50개까지 실제로 쌓이는데, 고정 오버헤드(상단바 4줄 + 제목줄 + 빈줄 = 6줄)를 빼면 24행 예산으로는 18개까지만 표시 가능 — 즉 한 세션에서 명령을 20개 남짓만 입력해도(흔한 사용 패턴) 오래된 이력이 사용자 모르게 사라지는, 이번 세션에 반복해서 찾은 것과 같은 "무제한 리스트를 안내 없이 잘라내는" 버그 클래스.
- 실사용 경로로 재현: localStorage의 `bbs_cmd_history`(앱이 부팅 시 이 키에서 `state.cmdHistory`를 그대로 읽어옴)에 30개짜리 이력을 주입하고 Playwright로 `/history` 화면을 렌더 — 1~17번만 보이고 18~30번은 안내 없이 사라짐을 확인.

구현:
- 고정 오버헤드 줄 수를 실제로 계산(`parts.join('\n').split('\n').length`)한 뒤 남는 예산만큼만 보여주고, 넘칠 때만 안내 줄(`... 외 N개 더 있습니다.`)을 추가 — 접속자 목록/첨부파일 목록과 동일한 slice+안내 패턴.
- 정확히 예산에 맞아떨어지는 경계(17개·18개)에서는 안내 줄 없이 전부 표시하도록 두 단계로 나눠 계산(불필요하게 안내 줄 자리를 낭비하지 않음).

검증:
- `node --check helpScreens.js` 통과.
- Playwright로 실제 앱(로컬 서버 + localStorage 이력 주입) 재현: 5/17/18개는 전부 표시(안내 없음), 30/50개는 17개만 표시 + 안내 줄 노출 — 수정 전(30개 주입 시 18개까지만 보이고 안내 없이 끊김)과 대비해 이제는 몇 개가 더 있는지 사용자가 알 수 있음.
- `npm run smoke:full-traversal`, `node scripts/smoke-mobile-viewports.js`(3개 뷰포트 × 27개 라우트), `npm run smoke:command-parity` 모두 통과.

결과: ✅ 완료 — 접속자 목록/첨부파일 목록/설문 선택지에 이어 네 번째로 발견한 "무제한(또는 예산 초과) 리스트를 안내 없이 잘라내는" 버그. 다음 라운드는 8번째 각도를 탐색해야 한다.
...
목표: 사용자가 `/account`(이용 현황, ACCT) 화면 스크린샷과 함께 "여기도 아래글자 잘려있어"라고 제보 — sysop 계정으로 마지막 줄("접속 시간·요금은 집계하지 않습니다")이 잘려 보임.

과정: `buildMyStatsAnsi`(systemAnsiBuilders.js)를 직접 함수 호출로 실측 — 이 화면은 help/policy와 달리 페이징 캔버스 패딩이 아예 없이(`renderSystemAnsiScreen`이 내용 그대로 렌더) 콘텐츠 줄 수가 곧 렌더 줄 수다. sysop 계정 데이터로 정확히 **23줄**이 나왔다 — 방금 전(20260726_2330) policy에서 확정한 "실기기가 헤드리스보다 근소하게만 커도 여유 없는 23줄은 잘린다"는 것과 완전히 동일한 경계값. 원인을 추적하니 `buildTopHeader()`가 이미 자체적으로 끝에 빈 줄 하나를 포함해 반환하는데(topLine/headerLine/구분선/빈줄 4줄), `buildMyStatsAnsi`가 그 직후에 `parts.push('')`로 빈 줄을 또 하나 추가해 헤더 아래 빈 줄이 중복으로 두 번 나오고 있었다 — 이 중복 자체가 정확히 1줄만큼의 낭비였다.

구현: 중복된 `parts.push('')`를 제거해 23줄 → 22줄로 줄였다(제거해도 정보나 시각적 구분 손실 없음 — buildTopHeader 자체 빈 줄이 이미 동일한 간격을 제공). 다만 닉네임이 최대 길이(20자, 표시폭 40칸)라 2줄로 접히면 다시 23줄이 될 수 있어, policy와 동일한 `overflow-y:auto` 안전망도 `body[data-screen="my-stats"] #terminal-screen`에 함께 추가(2차 방어선).
검증: 실측 재현(sysop 계정 데이터)으로 23→22줄 감소 확인. `node --check` 통과, CSS 중괄호 균형 확인. `smoke:full-traversal`/`smoke-mobile-viewports.js` 전부 통과.
결과: ✅ 완료 — policy 수정 직후 곧바로 다른 화면에서 완전히 동일한 근본 원인(중복 빈 줄로 인한 23줄 여유 없음)이 재현된 사례. 이번엔 원인이 코드 안의 실제 중복(불필요한 빈 줄)이라 재발 여부와 무관하게 확실히 개선되는 수정이었다.

## [2026-07-26 23:30] 사용자 재지적("아직도 마찬가지로 잘렸는데") — policy overflow-y:auto 수정이 스크린샷상 눈에 보이는 변화를 만들지 못한 근본 원인 파악, 페이지당 줄 수를 줄여 여유 확보

**LOG_ID: 20260726_2330**
목표: 사용자가 동일한 `/policy/tos` 화면을 다시 스크린샷으로 제보 — 직전(20260726_2230) `overflow-y:auto` 수정을 이미 배포 확인(prod style.css에 반영 확인)했음에도 "아직도 마찬가지로 아래 글자가 잘렸는데"라고 재지적.

재검토: `overflow-y:auto`는 잘린 내용을 스크롤로 "볼 수 있게" 만들 뿐, 스크롤을 하지 않은 최초 화면(스크린샷이 찍히는 바로 그 상태)의 렌더 결과 자체는 수정 전후로 픽셀 한 개도 달라지지 않는다 — 그래서 스크린샷만으로는 수정이 "안 먹힌 것"처럼 보인다. 근본 원인을 다시 짚어보면: `#terminal-screen`은 모바일에서 `height:auto`(flex-shrink 가능한 콘텐츠 기반 높이)라 컨테이너 자체엔 고정 픽셀 예산이 없다 — 진짜 제약은 바깥쪽 `100dvh` 뷰포트다. 실기기의 주소창/안전영역 등으로 실제 가용 높이가 이 앱이 가정한 것보다 근소하게 부족하면, `#terminal-screen`(flex:0 1 auto)이 자신의 콘텐츠 높이보다 눌려 찌그러지고, 그 초과분이 자기 자신의 `overflow:hidden`에 잘린다 — 직전 수정의 `overflow-y:auto`가 정확히 이 상황을 스크롤 가능하게 만들어주지만, 화면이 처음 그려진 그 순간의 모습은 그대로다.

구현: `policyScreens.js`의 `linesPerPage`를 19 → 18로 한 줄 줄였다. 헤더(4줄) + 본문(18줄) = 22줄이 되고, 캔버스 패딩 루프가 채우는 나머지 한 줄은 항상 빈 줄이 된다 — 여유가 부족해 무언가 눌려 잘리는 상황이 오더라도, 잘리는 건 이제 그 빈 줄이지 실제 문장이 아니다(1줄만큼의 실질적인 여유 픽셀을 벌어들이는 효과). 총 페이지 수는 16→17쪽으로 하나 늘지만 정보 손실은 없다.
검증: `linesPerPage=18` 적용 후 1쪽 마지막 줄이 "2. 이용계약 : 서비스 이용과 관련하여 PC통"에서 끊기고, 이어지는 "신동호회 01410와..." 문장은 2쪽으로 정상 이동함을 확인(페이지 라벨 "01/17"로 갱신 확인). `smoke:full-traversal`/`smoke-mobile-viewports.js`/`smoke:command-parity` 전부 통과.
결과: ⚠️ 부분 완료 — 헤드리스 환경은 뷰포트가 충분히 커서(실측 839px vs 콘텐츠 필요 높이 약 630px) 애초에 이 압착 현상 자체가 재현되지 않아, 이번 수정이 실기기에서 실제로 효과가 있는지는 헤드리스로 확정 검증할 수 없다(이 세션 전체에서 반복 확인된 헤드리스-실기기 측정치 격차 문제). 사용자에게 실기기 재확인을 요청한다.
...
목표: 사용자가 실기기 스크린샷(`/policy/tos`, 이용약관 1/16쪽)과 함께 "글 아래부분이 일부잘렸어"라고 제보. 헤드리스(Pixel 7 디바이스 프로필 포함) 재현 시도에서는 본문 마지막 줄과 컨테이너 경계가 정확히 일치해 잘림이 재현되지 않았다.

과정: 사용자에게 재현성을 확인(매번 같은 자리에서 재현됨, 이미 다른 화면을 둘러본 뒤였음 — 폰트 로딩 타이밍/일회성 렌더 글리치 가설 배제)한 뒤, `style.css`에서 `#terminal-screen`(고정폭 그리드 프레임) 관련 규칙을 전수 검색. 이 코드베이스엔 이미 잘 확립된 반복 버그 패턴이 있었다 — 화면마다 본문 줄 수가 콘텐츠에 따라 가변적일 수 있는데(`news-list`/`news-view`/`help`/`omok-play`/`chat-room`/`vote-detail`/`scramble-play` 전부 이 이유로 순차적으로 추가된 화이트리스트, 각각 별도 LOG_ID로 기록됨), 전역 규칙은 `#terminal-screen{overflow:hidden}`(23~24줄 "한 프레임 고정" 원칙)이라 그 화이트리스트에 없는 화면은 예산을 넘기면 스크롤 없이 그대로 잘린다. `policyScreens.js`의 `buildPolicyAnsi`는 `linesPerPage=19`로 **help와 완전히 동일한 페이징 로직**을 쓰고 있고, help는 이미 2026-07-18(LOG_ID 20260718_2340)에 정확히 이 증상으로 화이트리스트에 추가된 전력이 있는데도, `policy`(이용약관/개인정보처리방침) 화면만 그 목록에 함께 들어가지 못하고 누락되어 있었다 — 실기기 폰트가 헤드리스보다 근소하게 더 크게 렌더링되며 여유 없이 딱 맞춰둔 23줄 예산을 넘긴 결과로 판단(과거 여러 화면에서 동일 패턴으로 반복 확인된 원인).

구현: `public/style.css`의 `body[data-screen="..."] #terminal-screen{overflow-y:auto; ...}` 화이트리스트 셀렉터 목록에 `body[data-screen="policy"]`를 추가 — help 등 기존 6개 화면과 동일한 처리(내부 스크롤 허용, 스크롤바는 시각적으로 숨김, overscroll-behavior로 바운스 차단).

검증: Playwright(Pixel 7 디바이스 프로필)로 `getComputedStyle(#terminal-screen).overflowY`가 `policy` 화면에서 `hidden`(수정 전 전역 기본값) → `auto`(수정 후)로 바뀜을 확인. `smoke:full-traversal`/`smoke-mobile-viewports.js`(policy/tos 라우트 포함 3뷰포트×27라우트) 전부 통과.
결과: ✅ 완료 — CLAUDE.md에도 명시된 이 저장소의 대표적인 반복 버그 클래스("화면별 whitelist 확장 패턴에서 신규/유사 화면 하나가 누락")의 8번째 사례. 헤드리스 재현이 정확히 실패한 것 자체가 실마리였다 — 경계가 여유 없이 딱 맞아떨어진다는 것은 실기기의 근소한 렌더링 차이만으로도 쉽게 깨질 수 있다는 뜻이었다.

## [2026-07-26 22:00] "다른 화면도 더 훑어봐" 후속 조사 — 직전(20260726_2130) 상단바 수정이 잘못된 진단이었음을 발견, 실제 근본 원인으로 재수정

**LOG_ID: 20260726_2200**
목표: 사용자가 "다른 화면도 더 훑어봐줘"라고 요청 — 직전 라운드에서 고친 compact 상단바 폭(`max-width:100%`) 수정이 다른 화면(main/plaza)에서도 잘 적용되는지 재확인하다가, `news-view`(실제 뉴스 기사 화면, 목록이 아님)에서 상단바(100%, 393px)와 본문(여전히 44ch, 330px)이 반대 방향으로 어긋나는 새 불일치를 발견 — 직전 수정이 회귀를 만들었음을 스스로 포착.

재조사: `.getBoundingClientRect()`로 컨테이너 `<div>` 폭만 비교했던 직전 라운드의 측정이 근본적 착각이었음을 확인 — block 요소는 내용과 무관하게 항상 컨테이너 폭을 그대로 보고하므로, 실제 글자가 어디까지 그려지는지는 알려주지 않는다. `Range.getBoundingClientRect()`로 실제 렌더된 44칸짜리 구분선 텍스트 자체를 재측정한 결과, 이 폰트에서 44칸은 컨테이너 폭(391px)과 무관하게 항상 약 330px만 차지했다 — 즉 컨테이너를 100%로 넓혀도 그 안의 글자(왼쪽 정렬)는 여전히 330px만 쓰고 나머지 61px는 오른쪽에 빈 여백으로 남는다. `git log -S`로 이 저장소 히스토리를 뒤져보니, 이 정확한 근본 원인이 2026-07-21에 이미 한 번 발견·해결된 적이 있었다(commit 3d22688, `body[data-screen="post-view"] .ansi-screen-body { max-width:44ch; margin:0 auto; }`) — 당시 "컬럼 수를 늘려 실제 폭에 맞추는" 시도는 실기기 실측 오차로 문장이 화면 밖으로 잘려나가는 더 심각한 회귀를 냈다는 기록이 남아있어, 안전한 해법(컨테이너를 실제 텍스트 폭에 맞춰 줄이고 가운데 정렬)이 이미 정해져 있었다. 문제는 이 안전한 해법이 `post-view`·`news-view` 딱 두 화면에만 국소적으로 적용되고, 나머지 화면(main/plaza/help 등 대부분)의 일반 규칙에는 반영되지 않은 채 방치되어 있었던 것 — 그래서 이번에 다시 같은 버그가(help 화면에서) 제보된 것이다. 직전 라운드의 "상단바를 본문에 맞춰 100%로 늘리는" 수정은 정반대 방향(이미 검증된 해법과 반대로, 안 고쳐진 쪽 규칙에 상단바를 맞춤)이었다.

구현: `public/style.css` — 직전 라운드에서 추가한 상단바 `max-width:100%` 규칙을 제거하고, 대신 일반 `.ansi-screen-body`(모바일 미디어쿼리 내)를 `max-width:100%`에서 `max-width:44ch; margin:0 auto;`로 변경 — post-view/news-view에만 있던 검증된 해법을 전체 화면에 일반화. 단, `myinfo` 화면은 기존 주석(20260722_2400)이 명시하듯 44칸 고정폭 ANSI 격자가 아니라 `.myinfo-row`(`grid-template-columns:12ch 1fr`) 같은 자유 폭 HTML 패널이라 44ch 제한이 부적절 — `.ansi-screen-body[data-screen-kind="myinfo"] { max-width:100%; }`로 별도 예외 처리(셀렉터 특정도로 일반 규칙보다 우선 적용).

검증: `Range` 기반 실측으로 상단바 구분선(왼쪽 31.5~오른쪽 361.5)과 본문 구분선(왼쪽 32.5~오른쪽 362.5)이 1px 오차로 일치함을 확인. help/plaza/main/chat/weather/agora 등 다수 화면 스크린샷으로 상단바-본문 좌우 여백이 전부 일치함을 육안 확인(사용자가 원래 보낸 스크린샷과 동일한 비례). myinfo는 100% 폭으로 정상 유지됨을 별도 확인. 데스크톱(1280px, `data-layout-mode="full"`)은 상단바·본문 모두 기존 `calc(80ch+32px)` 그대로라 무관함(회귀 없음). `smoke:full-traversal`/`smoke-mobile-viewports.js`(3뷰포트×27라우트)/`smoke:renderer-ui`/`smoke:menu-wiring` 전부 통과.
결과: ✅ 완료 — 직전 라운드의 잘못된 진단을 사용자의 "다른 화면도 훑어봐" 요청 덕분에 스스로 발견·정정했다. 최종적으로는 이미 이 저장소에 2026-07-21부터 존재하던, 실기기 검증까지 마친 안전한 해법을 전체 화면에 일반화하는 것이 정답이었다 — 새 방식을 고안하는 대신 기존 검증된 패턴을 찾아 넓히는 편이 더 안전했다.

## [2026-07-26 21:30] 사용자 제보 스크린샷(모바일 도움말 화면) 기반 "왼쪽 치우침" 버그 2건 발견·수정

**LOG_ID: 20260726_2130**
목표: 사용자가 모바일(393px) `/help` 화면 스크린샷과 함께 "모바일화면 곳곳에 왼쪽 치우침이 보여"라고 제보 — 실제 스크린샷을 Playwright로 동일 뷰포트에서 재현하며 원인을 특정.

발견 1(핵심) — compact 상단바 폭이 본문과 어긋남: `.ansi-screen-body`는 `@media (max-width:768px)`에서 `max-width:100%`로 풀려 화면 가장자리까지 꽉 차는데(의도된 설계), `.retro-topbar--ansi[data-layout-mode="compact"] .retro-topbar-row1/row2/hr`는 이 미디어쿼리 밖의 고정 규칙(`max-width:44ch`)만 적용받아 44ch(이 폰트 393px 뷰포트 기준 330px)로 고정된 채 남아있었다. 실측(Playwright, 393px): 상단바(로고박스+시계, HELP/도움말/(01/07), 구분선)만 좌우 31.5px씩 안으로 들어가 중앙에 떠 있고, 바로 아래 본문(구분선·명령 목록)은 가장자리까지 꽉 차 있어 상단바-본문 경계에서 좌측 여백이 어긋나 보였다 — 상단바가 있는 모든 모바일 화면에서 재현되는, 이번 세션 처음 발견하는 새 버그 클래스.
발견 2 — `wrapAnsiText`의 줄바꿈 경계 공백 처리 누락: 이번 세션 18건 이상의 폭 버그 수정에 반복 의존해온 핵심 공용 함수가 순수 글자 수 기준으로만 줄을 자르며, 줄바꿈 경계가 하필 공백 바로 다음이면 그 공백을 다음 줄로 그대로 넘겨 해당 줄만 한 칸 밀려 보였다. 실측: help 화면 N/A 행 — 원문 "...글을 읽습니다."의 줄바꿈 지점이 정확히 "글을"과 "읽습니다" 사이 공백이라 두 번째 줄이 " 읽습니다."(앞에 공백 하나)로 표시됨. 사용자가 제보한 것과 동일한 스크린샷 안에서 함께 재현된, 내용에 따라서만 나타나는(GO/Z/F/Q 행은 우연히 경계가 공백과 안 겹쳐 무사했던) 별개의 버그.

구현:
1. `public/style.css` — `@media (max-width:768px)` 블록에 `.retro-topbar--ansi[data-layout-mode="compact"] .retro-topbar-row1/row2/hr { max-width:100%; }` 추가, 본문과 동일한 폭 규칙 적용.
2. `public/js/core/ansiBuilderUtils.js`의 `wrapAnsiText` — 줄바꿈 경계에 걸린 문자가 공백이면 그 한 칸을 다음 줄로 넘기지 않고 소비하도록 수정(표준 워드랩 동작).

검증: 사용자가 보낸 정확히 그 화면(모바일 `/help`)을 Playwright(393px)로 재현 — 수정 전 상단바 rect(x:31.5~361.5, width:330) vs 본문 rect(x:0~393, width:393) 불일치 확인, 수정 후 둘 다 x:0~393로 완전히 일치함을 실측 확인. N/A 행 두 번째 줄의 불필요한 선행 공백도 스크린샷 비교로 해소 확인. main/plaza 등 다른 모바일 화면에서도 상단바-본문 폭 일치 재확인, 데스크톱(1280px, `data-layout-mode="full"`)은 두 규칙 모두 원래부터 `calc(80ch+32px)`로 동일해 무관함(회귀 없음)을 확인. `node --check` 통과. `smoke:full-traversal`/`smoke-mobile-viewports.js`(3뷰포트×27라우트)/`smoke:renderer-ui`/`smoke:menu-wiring` 전부 통과.
결과: ✅ 완료 — 사용자가 스크린샷으로 제보한 화면에서 서로 독립적인 원인의 버그 2건(CSS 미디어쿼리 누락 1건 + 공용 줄바꿈 함수 로직 누락 1건)을 모두 찾아 수정. 두 버그 모두 상단바가 있는 화면 전반(1번) 또는 `wrapAnsiText`를 쓰는 화면 전반(2번, 텍스트 내용이 공백에서 정확히 줄바꿈되는 경우에 한함)에 걸쳐 있어 파급력이 크다.
...
목표: `/loop 모바일ui가 완벽해질때까지 전수조사` 계속 — 직전 이모지 폭 수정(20260726_1615)의 후속 점검: `isWideChar`가 이모지를 광폭(2칸)으로 분류하면서 `ansiEngine.js`의 `.wc` 클래스(`width:2ch; overflow:hidden;`) 박스 안에 렌더링될 텐데, 이 폰트 실측상 이모지 글리프 자연폭(18.72px)이 2ch 박스(15px)보다 넓어 `overflow:hidden`에 의해 글리프 자체가 시각적으로 살짝 잘리지 않는지 확인.
확인: 폰트 크기를 32px로 확대해 스크린샷으로 직접 관찰 — `.wc` 박스(32px, scrollWidth 40px, 약 20% 초과)로 인해 이모지 가장자리가 근소하게(각 변 약 10%) 잘리지만, 😀/🎉 모두 여전히 명확히 알아볼 수 있는 수준이었다. 완벽한 정밀도(비정수/3단계 폭 시스템)를 추구하려면 `fitCell`/`wrapAnsiText`/`.wc` CSS 전반의 이진 폭 아키텍처를 뜯어고쳐야 하는데, 이는 수정 전(레이아웃 전체가 밀려나 콘텐츠가 조용히 사라지는 심각한 오버플로) 대비 지금(글리프 가장자리만 근소하게 시각적으로 잘림)의 이득이 압도적으로 크므로, 추가 변경 없이 현재 상태를 의도적으로 수용한다.
결과: ✅ 완료 — 새 버그는 없었다(직전 수정의 알려진, 수용 가능한 부작용을 시각적으로 확인하고 의도적으로 그대로 두기로 결정). 순수 텍스트/레이아웃 폭 버그 클래스는 이번 라운드까지 포함해 매우 철저히 소진된 상태로 판단된다.

## [2026-07-26 17:00] [/loop 계속] 터치 타겟 크기(오목 핫스팟) 점검 — 결함 아닌 근본적 설계 제약으로 판정

**LOG_ID: 20260726_1700**
목표: `/loop 모바일ui가 완벽해질때까지 전수조사` 계속 — 지금까지 다루지 않은 완전히 새로운 관점(텍스트 폭이 아니라 터치 타겟 크기)으로 전환. 320px 폭에서 오목(15x15=225칸)·15퍼즐 핫스팟의 실제 탭 가능 크기를 측정.
확인: 오목 핫스팟은 320px 폭에서 24×24px로 측정됐다 — Apple HIG/WCAG 2.5.5가 권장하는 최소 44×44pt에 못 미친다. 그러나 15×15 보드를 어떤 폰 화면 폭(15×44=660px는 어떤 실기기 폭보다도 큼)에도 44px 셀로 담는 것은 원천적으로 불가능하며, 실제 바둑/오목류 모바일 앱 대부분이 동일한 제약을 갖는다 — 이미 이번 세션 앞선 라운드에서 실제 `.tap()`으로 탭 인식 자체는 정상 동작함을 확인했으므로(코딩 결함이 아니라 15x15 격자를 작은 화면에 담는 데서 오는 근본적 설계 트레이드오프), 수정 대상으로 보지 않는다. 15퍼즐은 핫스팟이 0개로 나와 최초엔 의심했으나, 코드 확인 결과 이 게임은 애초에 탭이 아니라 "옮길 숫자 입력(1~15) >>" 텍스트 입력으로 설계된 게임이라 정상 동작이었다(오작동 아님).
결과: ✅ 완료 — 새 버그는 없었다(설계 제약과 정상 동작 확인). 텍스트 폭 버그 클래스 소진 이후 처음 시도한 완전히 다른 범주(터치 타겟)에서도 실질적 결함은 발견되지 않았다.

## [2026-07-26 16:45] [/loop 계속] 검색어 하이라이트(highlightText) 재검토 — 새 버그 없음, 세션 전체 마무리 시점 재평가

**LOG_ID: 20260726_1645**
목표: `/loop 모바일ui가 완벽해질때까지 전수조사` 계속 — 게시판 검색 결과에서 검색어를 강조 표시하는 `highlightText()`(ANSI 색상 코드로 일치 부분을 감싸는 함수)가 `wrapAnsiText`/`fitCell`의 폭 계산과 올바른 순서로 결합되는지 점검.
...
목표: `/loop 모바일ui가 완벽해질때까지 전수조사` 계속 — 직전 라운드 이모지 폭 수정(`isWideChar`)이 `wrapAnsiText`(줄바꿈) 외에 `fitCell`/`fitCellEllipsis`(게시글 목록 제목 절삭 등에서 쓰는 말줄임표 로직)에서도 안전한지 후속 검증, 이어서 아직 깊이 들여다보지 않았던 전투 게임(Battleship) 빌더를 재검토.
확인: (1) 이모지가 섞인 80칸짜리 제목을 `fitCellEllipsis(title, 20)`로 절삭 — 서로게이트 쌍이 중간에 깨지지 않고 정확히 `displayWidth()===20`으로 잘렸다("🎉축하합니다 오늘의…"). 기존 for-of 순회 방식이 이미 서로게이트 쌍을 원자 단위로 다뤄 왔기 때문에 절삭 위치 계산에서도 손상 없이 안전하게 확장됨을 확인. (2) `buildBattleAnsi`(전투 게임)는 격자·안내문구 모두 `isMobile`/`wrapAnsiText` 분기가 이미 갖춰져 있었다(주석 확인 결과 이전 세션 라운드에서 이미 수정된 영역).
결과: ✅ 완료 — 새 버그는 없었다. 직전 이모지 수정의 안전성을 다른 경로(절삭)에서도 재확인하고, 남은 아케이드 게임 중 미점검 영역(전투)도 이미 안전함을 확인한 검증 라운드.

## [2026-07-26 16:15] [/loop 계속] 이모지 폭 미계산 — displayWidth()가 전 화면에 걸쳐 이모지를 좁은 문자로 잘못 계산하던 버그 수정, 28번째

**LOG_ID: 20260726_1615**
목표: `/loop 모바일ui가 완벽해질때까지 전수조사` 계속 — 4연속 검증(무버그) 라운드 이후 완전히 새로운 탐색 축으로 전환: 닉네임(서버 `maxLength:20`은 UTF-16 코드유닛 기준이라 서로게이트 쌍인 이모지가 최대 10개까지 통과)·채팅 메시지(최대 2000자)처럼 이모지가 섞일 수 있는 자유 텍스트에서 `displayWidth()`/`isWideChar()`(이번 세션 18건 이상의 폭 계산 버그를 고치는 데 반복적으로 의존해 온 핵심 공용 함수)가 이모지를 올바르게 계산하는지 점검.
발견: `isWideChar()`(`ansiRenderUtils.js`, 앱 전체가 공유하는 유일한 정의부)가 이모지 대역(U+1F000-1FAFF: 😀🎉👍🔥 등 대부분의 현대 이모지)을 전혀 다루지 않아 전부 좁은 문자(1칸)로 계산되고 있었다. 실측 결과 이 폰트 환경에서 이모지 하나는 아스키(7.5px)의 약 2.5배(18.72px, 한글 15px보다도 넓음)로 렌더링되는데, `displayWidth()`가 1칸으로 셈하니 `wrapAnsiText`가 실제보다 훨씬 많은 이모지를 한 "줄"에 채웠다 — 재현: 동일 이모지 32개로 구성된, `displayWidth()` 계산상 "32칸"인 줄이 실제로는 390px 뷰포트에서 614px, 224px(57%) 오버플로. 이 함수는 이번 세션 프로필/멤버검색/채팅/투표/게시글 등 18건 이상의 수정이 공통으로 의존해 온 핵심 함수라, 닉네임·채팅 메시지·게시글 제목/본문 등 이모지가 들어갈 수 있는 모든 자유 텍스트 경로에 잠재된 버그였다.
구현: `isWideChar()`에 `cp >= 0x1F000 && cp <= 0x1FAFF` 범위를 광폭(2칸)으로 추가 — 단일 정의부 수정이라 `createAnsiBuilderUtils`를 쓰는 모든 ANSI 빌더, `ansiEngine.js`의 실제 HTML 렌더링, 핫스팟 좌표 계산까지 전부 자동으로 적용된다. 실측(2.5배)에 정확히 맞추려면 폭 시스템 자체를 정수 2단계(좁음/넓음)에서 비정수/3단계로 확장해야 하는데, 이는 `fitCell`/`wrapAnsiText`/`.wc`(CSS 고정 2ch) 등 이 세션 전체가 의존해 온 이진 폭 아키텍처를 건드리는 과도한 변경이라 판단 — 업계 관행(대부분의 터미널·wcwidth 구현이 이모지를 2칸으로 취급)에 맞춰 2칸으로 분류했다.
검증: Node 하네스로 기존 분류(박스 문자 ─, 오목 돌 ○●, 한글 가, 아스키 A) 회귀 없음 확인. 수정 전/후 비교 — 순수 이모지 32개 스팸(극단적 최악의 경우)은 57% 오버플로에서 5.6%(44칸 전체가 이모지인 줄만 해당, 실제 텍스트가 섞이면 이 잔여값도 사라짐)로 대폭 개선. 실제 대화에서 흔한 "한국어+이모지 혼합" 메시지("오늘 진짜 재밌었어요 😀 다음에 또 만나요~ 🎉 ...")로는 수정 전/후 모두 오버플로 없음을 확인해, 이 버그가 이모지 밀도가 매우 높은 극단적 사례에서만 실제로 재현됨을 함께 문서화했다. `npm run smoke:full-traversal`/`smoke:renderer-ui`/`smoke:menu-wiring`/`smoke:chat-rooms`/`scripts/smoke-mobile-viewports.js`(3뷰포트×27라우트) 전부 통과.
결과: ✅ 완료 — 이번 세션 28번째 버그. 텍스트 절삭/줄바꿈 자체가 아니라 폭 "계산"이 처음부터 틀렸던, 지금까지와는 다른 근본 원인(문자셋 커버리지 공백)의 버그였다. 단일 공유 함수 수정으로 이 세션에서 다뤄온 거의 모든 화면(프로필/닉네임/채팅/투표/게시글/멤버검색 등)에 자동으로 적용된다는 점에서 파급력이 크다. 완벽한 폭 정밀도(2.5배)에는 못 미치지만(2배로 근사), 이는 아키텍처 침습성 대비 실익을 고려한 의도적 절충이다.

## [2026-07-26 16:00] [/loop 계속] 모바일 키보드 등장 시나리오 점검 — 테스트 방법론 오탐 확인, 기존 코드가 이미 방어 중임을 재확인

**LOG_ID: 20260726_1600**
목표: `/loop 모바일ui가 완벽해질때까지 전수조사` 계속 — 새 탐색 축으로, 직전 라운드에서 고친 인라인 프롬프트(scrollIntoView)가 모바일 키보드가 뜬 뒤(가용 세로 공간이 한 번 더 줄어드는 시점)에도 여전히 유효한지 점검.
과정: 스크램블 게임 렌더 완료 후 `page.setViewportSize()`로 뷰포트를 추가로 줄이고 `window.resize` 이벤트를 발생시켜 키보드 등장을 흉내냈다 — 처음엔 `cmdInputBottom`이 뷰포트 밖으로 나가는 것처럼 보였다(오탐 가능성 인지 전 1차 결과).
재검토 및 정정: `terminalViewportMetrics.js`를 읽어보니 이 앱은 `window.visualViewport`의 실제 리사이즈 이벤트를 기준으로 키보드 등장을 감지(`keyboardInset >= 96`)하고, 감지되면 `keyboardJustOpened` 분기에서 `screenEl.scrollTop = screenEl.scrollHeight`(맨 아래로 스크롤 — post-write 화면만 예외)를 이미 명시적으로 수행하고 있었다(LOG_ID 20260725_1645/1800, "전수조사로 발견"이라는 기존 주석으로 봐도 이미 한 차례 점검된 영역). 그런데 헤드리스 Playwright는 실제 가상 키보드가 없어 `window.visualViewport.height`가 `setViewportSize`만으로는 실제 기기처럼 줄어들지 않는다 — 내 시뮬레이션은 이 앱이 실제로 감지에 쓰는 신호(visualViewport resize)가 아니라 `window.resize`만 발생시켰으므로 `keyboardJustOpened` 분기가 애초에 발동하지 않았다. 즉 1차 결과는 실제 버그가 아니라 테스트 방법론의 한계로 인한 오탐이었다.
결과: ✅ 완료 — 새 버그 없음(오탐 정정). 코드 리뷰로 이 시나리오가 이미 의도적으로 처리되어 있음을 확인했고, 헤드리스 환경에서 실제 가상 키보드에 의한 visualViewport 축소는 신뢰성 있게 재현할 수 없다는 테스트 방법론의 한계를 문서화한다 — 향후 이 각도의 재현 시도는 실기기 검증이 아니면 오탐 위험이 있다는 점을 남긴다.

## [2026-07-26 15:45] [/loop 계속] 채팅방·아케이드 핫스팟 재검토 — 새 버그 없음, 세 번째 연속 정체 라운드

**LOG_ID: 20260726_1545**
목표: `/loop 모바일ui가 완벽해질때까지 전수조사` 계속 — (1) 대화방(chat-room)이 320px에서도 안전한지, (2) 날씨 메뉴에서 발견했던 "핫스팟 좌표가 고정 컬럼수(80)를 가정"하는 패턴이 다른 아케이드 게임(오목/오델로/15퍼즐)에도 남아있는지 점검.
확인: (1) 대화방은 이번 세션 앞선 라운드(복합 스트레스 테스트, 390px)에서 이미 실제 긴 메시지로 검증됐고, 이번에 스크램블/토정비결에서 반복 검증한 것과 동일한 CSS 메커니즘(`overflow-y:auto` 스코프 규칙)을 그대로 재사용하므로 별도 재현 없이도 안전하다고 판단(실제 방 생성 재현은 이미 검증된 동일 메커니즘에 대한 중복 검증이라 생략). (2) `contentWidth / 숫자` 형태의 하드코딩 컬럼수 가정을 전체 코드베이스에서 재검색 — 날씨 메뉴(이미 수정됨) 외에는 없음을 확인. 오목 핫스팟(`renderOmokBoardHotspots`)은애초에 `measureLineSegmentBounds`(실제 텍스트 위치 측정 방식)를 쓰고 있어 이 버그 클래스에 안전한 설계였다.
결과: ✅ 완료 — 새 버그는 없었다. 이번 세션에서 320px/568x320 조합으로 발견한 버그 클래스(스크램블→오락실→중앙화)에 대한 탐색이 이 라운드로 3연속 정체 상태에 도달했다 — 사용자의 "계속 반복" 지시가 유효한 한 다음 라운드도 계속하되, 다음 라운드는 다시 새로운 탐색 축(예: 접근성/브라우저 확대, 다른 화면군 재검토)으로 전환할 시점으로 판단된다.

## [2026-07-26 15:30] [/loop 계속] 랜드스케이프×초소형(568x320) 조합 스트레스 검증 — 새 버그 없음, mountPromptRow 중앙화 수정 견고성 확인

**LOG_ID: 20260726_1530**
목표: `/loop 모바일ui가 완벽해질때까지 전수조사` 계속 — 직전 라운드에서 320x568(세로)을 회귀 스위트에 추가했으니, 이번엔 같은 초소형 기기의 랜드스케이프 방향(568x320 — 세로 축이 320으로 더 짧아, 지금까지 고친 세로 오버플로 수정들에 가장 가혹한 조합)에서 전체 라우트가 안전한지 수동 스트레스 테스트.
방법: 27개 기본 라우트 + 오락실 6종(바이오리듬/운세/MBTI/혈액형/궁합/토정비결) 전체를 568x320에서 가로/세로 오버플로 동시 점검. 특히 직전 라운드에서 중앙화한 `mountPromptRow`의 `scrollIntoView` 수정이 가장 타이트한 이 조합에서도 실제로 버티는지 확인하기 위해, 토정비결에 생년월일을 입력해 실제 결과(12개월 목록)까지 진행하고, 스크램블 게임을 "L"(새 게임) 명령으로 4회 연속 재시작해 라운드마다 달라지는 힌트 단어 개수 변화에도 안정적인지 반복 검증.
확인: 33개 라우트 전부 가로/세로 오버플로 없음(`scrollWidth===innerWidth`, `cmdInputBottom<=innerHeight+1`). 토정비결 결과 `cmdInputBottom` 318.6/320, 스크램블 4회 재시작 각각 319.97/319.78/319.53/319.78 — 전부 320 문턱 바로 아래(오차 1px 이내)로, `block:'nearest'` 스크롤이 정확히 필요한 만큼만 동작해 이 극단적인 조합에서도 절대 넘치지 않으면서 여유도 거의 없이 딱 맞게 동작함을 확인 — 중앙화 리팩터링이 실제로 견고함을 실측으로 뒷받침했다.
결과: ✅ 완료 — 새 버그는 없었다. 지난 3개 라운드(스크램블 발견→오락실 확장→중앙화 리팩터링)의 결과물이 가장 가혹한 뷰포트 조합에서도 안정적임을 확인하는 검증 라운드.
...
목표: `/loop 모바일ui가 완벽해질때까지 전수조사` 계속 — 지금까지 이 세션의 모바일 스모크 테스트가 390x844/360x740 두 뷰포트만 검사해, 실기기 중 가장 좁은 축(iPhone SE 1세대급, 320px)은 한 번도 커버된 적이 없었다는 공백을 발견. `scripts/smoke-mobile-viewports.js`에 세 번째 뷰포트(320x568)를 추가.
발견: 추가 즉시 실제 버그를 하나 잡았다 — 스크램블(오락실) 게임에서 명령 입력줄(`#cmd-input`)이 뷰포트 아래로 벗어나 있었다(`cmdInputBottom` 593~665 vs `innerHeight` 568). 원인 추적 결과, 아케이드 게임 9종이 공유하는 `arcadeRender()`가 명령 입력을 하단 footer가 아니라 `screenEl`(스크롤 가능한 `#terminal-screen`) 내부에 `.game-prompt-host`로 인라인 마운트하는 구조였다 — 스크램블은 라운드마다 숨은 정답 단어 개수가 달라 힌트 목록 길이가 가변적이라(뷰포트 320x568 라운드마다 20~23줄), 힌트가 많은 라운드에서는 프롬프트가 화면 스크롤 영역 아래쪽에 위치하게 된다. 아케이드 게임은 모바일에서 키보드가 판 위로 불쑥 뜨는 걸 막기 위해 일부러 auto-focus를 하지 않는데, 그 결과 포커스에 따라오는 브라우저 기본 스크롤도 발동하지 않아 이 프롬프트가 화면 밖에 그대로 남아 있었다(실측: `isFocused: false`, `cmdInputDisabled: false` — 준비 상태와 무관하게 매번 재현되는 레이아웃 버그, 타이밍/레이스 아님을 재현 테스트로 확인).
구현: `arcadeScreens.js`의 `arcadeRender()`에서 `mountPromptRow(host)` 직후 `host.scrollIntoView({ block: 'nearest' })`를 추가 — 아케이드 게임 9종(오목/오델로/숫자야구/행맨/15퍼즐/스크램블/WP/타자연습/전투) 전체에 공통 적용되는 단일 지점 수정. `block:'nearest'`라 이미 보이는 경우(대부분의 게임·대부분의 라운드)엔 아무 것도 하지 않아 매 수(턴)마다 화면이 불필요하게 튀지 않는다. 같은 원인으로 `style.css`에 `body[data-screen="scramble-play"] #terminal-screen`을 기존 뉴스/도움말/오목/채팅/투표상세 그룹에 추가해 내부 스크롤도 함께 허용(1차 시도 — 이것만으로는 부족해 위 JS 수정이 실제 근본 수정).
검증: 동일한 재현 테스트(320x568, `isMobile:true`)를 8회 반복 실행 — 힌트 줄 수가 20/22/23으로 매번 달라져도 `cmdInputBottom`이 항상 567.53(뷰포트 568 이내)으로 안정적으로 고정됨을 확인(수정 전엔 8회 전부 593~665로 실패). `npm run scripts/smoke-mobile-viewports.js`(이제 3개 뷰포트×27라우트)를 연속 2회 실행해 안정적으로 전부 통과함을 확인(플레이키 아님). `npm run smoke:full-traversal`/`smoke:renderer-ui`/`smoke:menu-wiring`도 전부 통과.
결과: ✅ 완료 — 이번 세션 26번째 버그이자, 지금까지의 "가로 오버플로" 계열과 다른 새로운 하위 유형(스크롤 가능한 인라인 프롬프트가 뷰포트 밖에 남는 세로 방향 문제)이었다. 회귀 스위트 자체의 커버리지 공백(320px 미검사)을 메운 것이 이 발견을 가능하게 했다는 점에서, 버그 수정 이상으로 향후 회귀 방지 인프라 개선의 의미가 크다.
...
목표: `/loop 모바일ui가 완벽해질때까지 전수조사` 계속 — 지난 라운드까지 발견한 "하드코딩된 고정폭 화면이 isMobile을 무시" 패턴을 계속 찾기 위해 `ansiBoardBuilders.js`를 재검토하던 중, `state.theme === 'nownuri'`(사용자가 `SET THEME NOWNURI` 명령으로 직접 켤 수 있는 실제 선택 가능 테마)일 때만 실행되는 `buildNownuriMainMenuAnsi`/`buildNownuriGuideAnsi` 두 함수를 발견.
발견: 두 함수 모두 `targetCols` 매개변수를 받으면서도 본문(제목줄·3단 메뉴 그리드)은 전혀 참조하지 않고 80칸 하드코딩 그대로였다. Node 하네스로 직접 로드해 실측한 결과 — 대문 화면: 모바일(44칸)에서 최대 표시폭 93칸(예산의 2배 이상, 이번 세션 발견 중 최대 오버플로), **데스크톱(80칸)조차 제목줄이 93칸으로 이미 조용히 잘리고 있었다**(이 화면만 유일하게 모바일뿐 아니라 데스크톱에서도 실제로 깨져 있던 사례). 가이드 화면: 모바일 최대 77칸(데스크톱은 77<80이라 안전, 모바일 전용 결함). `state.theme==='nownuri'`는 대문(초기화면, 앱에서 가장 트래픽이 높은 화면)에 조건 없이 걸리므로, 이 테마를 켠 모든 모바일 사용자가 매번 겪는 버그였다.
구현: 두 함수 모두 `isMobile = targetCols < 60` 분기 추가. 제목줄은 `fitCellEllipsis(...).replace(/\s+$/g,'')`로 targetCols에 맞춰 잘라 데스크톱 오버플로까지 함께 해소. 대문 화면은 모바일에서 기존 3단 그리드를 1단 세로 목록(문번호별로 순서 유지, 섹션 제목 "[ 서비스 ]"/"[ 안내 ]"/"[ 인터넷 ]" 유지)으로 재배치, 가이드 화면도 동일하게 1단 재배치. 핫스팟 생성(`buildMenuHotspotsFromRows`)은 픽셀 좌표 계산이 아니라 렌더된 텍스트에서 "문번호. " 패턴을 줄 단위로 스캔하는 방식이라(직전 날씨 메뉴 버그와 다른 구조), 1단으로 바꿔도 각 항목이 그대로 인식됨을 코드로 확인 — 별도 좌표 수정 불필요. 데스크톱 분기는 제목줄 외 기존 3단 그리드·매직넘버 그대로 유지.
검증: Node 하네스(`createBoardAnsiBuilders` 실로드, `state.theme='nownuri'`)로 대문/가이드 화면을 모바일·데스크톱 각각 렌더 — 대문 모바일 최대 43칸(44 이하), 데스크톱 최대 78칸(80 이하, 제목줄 오버플로도 해소), 가이드 모바일 최대 44칸(정확히 예산 내), 가이드 데스크톱 최대 77칸(기존과 동일, 미변경 확인) — 전부 오버플로 0건. 실제 브라우저로 `SET THEME NOWNURI` 명령까지는 정상 적용을 확인했으나(body class/`data-theme` 반영 확인), 화면 갱신까지 이어지는 전체 왕복 검증은 사용자 판단(이 테마 기능 자체의 우선순위가 낮음)에 따라 생략하고 하네스 검증으로 갈음했다. `npm run smoke:full-traversal`/`smoke:menu-wiring`/`smoke:boards`/`scripts/smoke-mobile-viewports.js`(27×2) 전부 통과.
결과: ✅ 완료 — 이번 세션 21번째(대문)·22번째(가이드) 버그. 대문 화면은 모바일 예산의 2배 이상(93/44)을 넘기면서 데스크톱에서도 동시에 깨져 있던, 이번 세션 통틀어 가장 심각한 오버플로 폭이었다.

## [2026-07-26 13:00] [/loop 계속] 날씨 지역 선택 메뉴 — 2단 목록 고정폭 오버플로 + 핫스팟 좌표 고정 80컬럼 버그(19·20번째) 수정

**LOG_ID: 20260726_1300**
목표: `/loop 모바일ui가 완벽해질때까지 전수조사` 계속 — 지난 두 라운드에서 발견한 "하드코딩된 고정폭 다이얼로그가 isMobile을 무시" 패턴을 더 찾기 위해, `memoAnsiBuilders.js`(쪽지 보기 — 재검토 결과 라벨/값 전부 안전 확인, 새 버그 없음)를 먼저 점검한 뒤, `weatherAnsiBuilders.js`/`weatherScreens.js`(날씨 서비스)로 확장.
발견(1/2): `buildWeatherMenuAnsi`(날씨 지역 선택 메뉴)가 `isMobile`을 전혀 참조하지 않고, 구분선(52칸 고정)과 2단 지역 목록(이름 칸 20칸×2단 = "  "+문번호2+". "+이름20칸 을 좌우로 이어붙임, 표시폭 54칸)을 항상 데스크톱 전제로 렌더하고 있었다. 직접 계산으로 확인(구분선 표시폭 54 vs 모바일 예산 44, 지역 목록 줄도 동일하게 54) — **날씨 메뉴를 열 때마다 매번 100% 재현**되는 버그였다(같은 세션의 PDS 전송 박스·게시글 작성 구분선과 동일한 클래스).
발견(2/2): 같은 화면의 지역 클릭 핫스팟 좌표 계산(`renderWeatherRegionHotspots`)이 실제 렌더 컬럼수(모바일 44/데스크톱 80)와 무관하게 `cellW = contentWidth / 80`으로 항상 80 고정이었다 — 모바일은 44컬럼에 맞춰 폰트가 더 크게 스케일되므로 컬럼당 실제 픽셀폭이 desktop과 다른데, 이를 무시하고 항상 80분의 1로 계산해 핫스팟 클릭 영역이 실제 텍스트 위치와 어긋날 수 있는 잠재적 결함 — 이번 세션 처음 발견한, "오버플로"가 아닌 새로운 하위 버그 유형(핫스팟 좌표 고정폭 가정).
구현: `buildWeatherMenuAnsi`에 `isMobile`/`targetCols`를 계산해 구분선을 `targetCols - 4`로, 모바일에서는 2단 목록 대신 1단 목록(이름 칸은 기존 20칸 유지, 전체 폭 26칸으로 44칸 예산에 여유 있게 수렴)으로 전환. 반환값에 `isMobile` 플래그를 추가해 `renderWeatherRegionHotspots`/`renderWeatherMenuScreen` 호출 체인까지 전달, `cellW` 계산의 컬럼수(`cols = isMobile ? 44 : 80`)를 실제 렌더 컬럼수와 일치시키고, 모바일에서는 오른쪽 컬럼 핫스팟 생성을 건너뛰도록 함. 데스크톱 분기는 기존 로직·매직넘버(26/24/30 등) 그대로 유지.
검증: Node 하네스(실제 `createWeatherAnsiBuilders` 로드, ANSI 이스케이프 제거 후 순수 텍스트 폭 측정)로 11개 지역(서울~제주+내 위치) 데이터를 모바일/데스크톱 양쪽 렌더 — 모바일 최대 줄 폭 44(targetCols와 정확히 일치, 오버플로 0건), 데스크톱 최대 줄 폭 80(오버플로 0건) 확인. 로그인이 필요 없는 공개 서비스라 실제 브라우저(Playwright)로도 직접 검증 — `/service/weather` 진입 시 모바일 `scrollWidth===clientWidth`(오버플로 없음) 확인, 핫스팟 11개 생성 확인, 두 번째 핫스팟(문번호 1·서울)을 tap해 정확히 "서울특별시" 상세 화면으로 이동함을 확인(핫스팟 좌표 수정이 실제로 올바르게 동작). 데스크톱도 동일하게 클릭 시 기존 DOM 순서(좌→우 행 단위) 그대로 올바른 지역(전라북도, 2단 배치상 첫 행 우측 항목)으로 이동해 회귀 없음을 확인. `npm run smoke:rss-services`/`smoke:full-traversal`/`smoke:menu-wiring`/`smoke:renderer-ui`/`scripts/smoke-mobile-viewports.js`(27×2) 전부 통과.
결과: ✅ 완료 — 같은 세션의 19번째(오버플로)·20번째(핫스팟 좌표 고정폭) 버그. 특히 20번째는 지금까지의 "텍스트가 잘린다"는 시각적 버그 클래스와 달리, 화면은 멀쩡해 보여도 실제 탭 판정 위치가 어긋날 수 있는 조작감 버그로, 이번 세션에서 처음 발견한 새로운 하위 유형이다.

## [2026-07-26 12:45] [/loop 계속] 게시글 작성/수정 박스 에디터 구분선 — 글쓰기마다 100% 재현되는 고정폭(76칸) 오버플로 버그(18번째) 수정
...
**LOG_ID: 20260726_0340**
목표: `/loop 모바일ui가 완벽해질때까지 전수조사` 계속 — 게시글 상세보기(20260726_0210)·토론의 광장 안건 보기(20260726_0330)에서 연달아 발견한 "서버 허용 최대 길이 vs 화면 표시 폭" 버그 클래스가 쪽지(memo) 보기 화면에도 있는지 확인.
발견: 예상대로 재현 — `buildMemoViewAnsi()`의 "제목" 행이 `fitCell` 한 줄 절삭이었는데, 쪽지 제목은 서버에서 최대 60자까지 저장된다(`MemoRepositoryShared.js`). 실제로 60자 한글 제목의 쪽지를 API로 만들어(자기 자신에게 발송) 렌더 결과를 직접 확인 — 정확히 같은 방식으로 잘림.
조사 방법: 쪽지 보기는 로그인이 필요한 화면이라 이번 세션에서 이미 한 번 시도했던 "실제 로그인 세션 흉내내기"가 배보다 배꼽인 것으로 판명났던 전례(20260726_0250 회차)를 반복하지 않기 위해, 대신 `buildMemoAnsiBuilders.js`를 Node 하네스로 직접 로드해(ES 모듈 export/import 구문을 정규식으로 벗겨 `new Function`으로 평가하는 이 세션 전반의 확립된 패턴) 실제 API가 반환한 진짜 쪽지 데이터로 `buildMemoViewAnsi()`를 직접 호출·검증 — 브라우저 없이도 실제 렌더 함수의 출력을 그대로 확인할 수 있어 이번 사안엔 이 방법이 더 적합했다.
구현: 게시글/안건과 동일한 패턴 — "제목" 행만 `wrapAnsiText`로 여러 줄로 접고 라벨 폭만큼 들여쓰기. 보낸이/받은날 등 다른 행은 원래도 짧은 값이라 그대로 둠. 쪽지 목록 화면(`buildMemoListAnsi`)의 제목 칸은 한 줄 목록 행 절삭이 원래 의도된 동작이라 손대지 않음(회귀 없음, 코드 재확인으로 검증).
검증: `node --check` 통과. 하네스로 실제 60자 제목 렌더 결과 확인 — 수정 전 "가나다라마바사아자차가나다라마바사"(18자)에서 잘림, 수정 후 4줄로 완전히 표시됨. `npm run smoke:full-traversal` 재실행 통과. 테스트로 만든 쪽지는 실 Supabase API로 직접 삭제해 정리.
결과: ✅ 완료 — 같은 버그 클래스의 3번째 발생 사례. 이제 이 3개 화면(게시글/안건/쪽지 보기) 모두 "라벨: 긴 자유 텍스트" 패턴에 wrapAnsiText를 적용했다 — 남은 유사 화면이 있다면 다음 라운드에서 계속 점검.

## [2026-07-26 02:50] [/loop 계속] 토론의 광장(FORUM) 안건 보기 — 80자 안건 제목이 말줄임표 없이 잘려 보이던 버그 수정

**LOG_ID: 20260726_0330**
목표: `/loop 모바일ui가 완벽해질때까지 전수조사` 계속 — 토론의 광장(회의실/안건, `confAnsiBuilders.js`)을 실제로 회의실 생성→안건 발의→안건 보기까지 API로 콘텐츠를 만들어 점검(게시글 상세보기에서 발견했던 "제목 최대 길이 vs 화면 표시 폭" 유형의 버그가 이 화면에도 있을 가능성이 높다고 보고 우선 확인).
발견: 예상대로 재현 — `buildConfAgendaViewAnsi()`의 "안건" 행이 `row()` 헬퍼(고정폭 `fitCell` 한 줄 절삭)를 그대로 썼는데, 안건 제목은 서버에서 최대 80자까지 저장되고(`ConfRepositoryMemory.js`/Supabase 버전 동일) 글쓰기 쪽엔 길이 제한이 없다. 80자 한글 제목으로 실제 안건을 만들어 보기 화면에 들어가 보니 "안건 : 1. 가나다라마바사아자차가나다라마바"처럼 15자쯤에서 말줄임표 없이 그냥 잘렸다. 같은 화면의 발의자/재청/발의일 행은 원래도 짧은 값이라 문제없음 — 제목 행만의 문제.
구현: `row()`는 그대로 두고, 제목 전용 `rowWrapped(label, value)` 헬퍼를 추가해 `wrapAnsiText`로 여러 줄로 접되 라벨 폭만큼 들여쓰기해 이어지는 줄이 값 칸 아래에 정렬되도록 함.
검증: `node --check` 통과. 실제로 80자 제목 안건을 만들어(회의실 생성→안건 발의→안건 보기, 메모리 아닌 실행 중인 Supabase 백엔드 dev 서버) 모바일(390x844) 스크린샷 확인 — 5줄로 깔끔하게 줄바꿈되고 라벨 아래 정렬 유지, 잘림 없음. 회의실 목록/안건 목록 화면(제목을 `fitCell`로 한 줄 절삭하는 의도된 목록 표시)은 무회귀 확인. `npm run smoke:full-traversal`/`smoke:vercel-ready`/`smoke:auth-bridge` 재실행 통과. 테스트로 만든 회의실+안건은 실 Supabase 프로젝트 데이터라 서비스 롤 키로 직접 삭제해 정리.
결과: ✅ 완료 — 게시글 상세보기(20260726_0210)와 같은 유형의 버그를 토론의 광장에서도 발견해 수정. "서버가 허용하는 최대 길이 vs 화면이 실제로 표시할 수 있는 폭"을 화면마다 짝지어 확인하는 방법이 계속 유효함을 재확인.
