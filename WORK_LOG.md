## [2026-07-09 13:40] 뉴스 이전/다음(A/N) 단축키 루프 정방향(원래 스펙) 복원 및 경계 가드 교정

**LOG_ID: 20260709_1340**
목표: `A`(이전글, 기사 번호 작아짐)와 `N`(다음글, 기사 번호 커짐) 단축키의 약속된 기본 탐색 방향을 본래 스펙대로 복원하되, 페이징 목록 범위를 벗어날 때 발생하는 인덱스 초과 껑충 점프 현상만 깔끔히 제거한다.
변경 파일: `public/js/core/commandRouterService.js`
수행 작업:
1) [진단 및 분석] 
   - 기존 수정에서 `A`와 `N`의 기사 번호 증감 매핑 방향을 거꾸로 판단하여, 사용자가 기대하던 단축키 방향과 반대로 뒤집히는 역효과가 발생했다.
   - 원래 기대된 방향: `A` (이전글)는 이전 글 번호(더 작은 번호)로 이동하는 방향(인덱스 감소), `N` (다음글)은 다음 글 번호(더 큰 번호)로 이동하는 방향(인덱스 증가)이다.
2) [수정] 
   - `commandRouterService.js` 의 `cmd === 'A'` 및 `cmd === 'N'` 루프 탐색 방향을 본래 방향인 `A` ➡️ `skipIdx--` (인덱스 감소), `N` ➡️ `skipIdx++` (인덱스 증가)로 즉시 원복하였다.
   - 페이징 목록 경계 이탈 예외 가드(`currentIndex === -1`) 내의 타겟 기사 번호 탐색 방향 또한 이에 맞게 `N ➡️ currentNoNum + 1`, `A ➡️ currentNoNum - 1` 로 일치시켰다.
3) [검증] `npm test` 유닛 테스트 통과 확인.
실행: `npm test`
기대: 단축키 `A`와 `N`을 누르면 사용자가 의도한 원래 정방향 스펙대로 매끄럽게 전후 기사 조회가 수행된다.
결과: ✅ 완료
