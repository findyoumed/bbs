## [2026-07-18 23:55] 오늘의 운세(FORTUNE) — 안내 문구와 실제 계산 불일치 수정, 진짜 십이지 관계 구현

**LOG_ID: 20260718_2355**
목표: 사용자 지적("오늘의 운세 정확해?") 반영 — 안내 화면엔 "십이지의 삼합·육합·육충·육해 관계를 사용합니다"라고 써놓고, 실제 계산(`buildFortuneAnsi`)은 (생년+오늘 날짜)를 섞은 해시값 하나로 점수를 뽑는 가짜 로직이었다. 코드 전체에 삼합/육합/육충/육해 문자열이 이 안내 문구 한 줄 말고는 전혀 없었다.
구현:
- `amusementAnsiBuilders.js`에 실제 십이지 관계표 추가 — 삼합(신자진·사유축·인오술·해묘미), 육합(자축·인해·묘술·진유·사신·오미), 육충(자오·축미·인신·묘유·진술·사해), 육해(자미·축오·인사·묘진·신해·유술). ZODIAC 배열(0=쥐~11=돼지) 인덱스 기준.
- 그레고리력 날짜 → 율리우스일(JDN) 변환(Fliegel & Van Flandern 공식)과 `(JDN+1)%12`로 오늘의 일진(지지)을 계산 — 60갑자 전체가 아니라 관계 판정에 필요한 12지지 순환만.
- 띠(연지)와 오늘 일진(일지) 사이의 관계(삼합/육합/평운/비화/육충/육해)를 판정해 그 관계의 등급을 총운/애정운/금전운/건강운 점수의 기준값으로 쓰고, 날짜별 소폭 편차(-1~+1)만 더해 네 항목이 서로 무관하지 않고 오늘의 실제 관계를 중심으로 갈리게 했다.
- **기준일(60갑자 오프셋) 검증**: 오프라인이라 직접 만세력을 대조할 방법이 없어 WebSearch로 확인 — 2026-07-18은 실제로 "계사(癸巳)일"(지지=사=뱀)이라는 게 여러 출처(fateengineering.site, 중부일보 오늘의 운세 기사)에 공통 기재되어 있고, 이 공식으로 계산한 JDN(2026-07-18)=2461240 → (2461240+1)%12=5=뱀(사)과 정확히 일치 — 오프셋이 맞음을 확인.
검증: 관계표 자체를 노드로 독립 실행해 12지지 전부가 삼합 2개·육합 1개·육충 1개·육해 1개·평운 6개로 대칭 구조를 이루는지 확인. `npm run smoke:vercel-ready`, `npm run smoke:renderer-ui` 통과. 로컬 dev 서버에서 1990/2000/1984/1996년생으로 실측 — 1990년생(말띠)은 오늘(말날)과 "비화", 2000년생(용띠)은 "평운", 1984·1996년생(둘 다 쥐띠)은 오늘(말날)과 "육충"(자오상충, 잘 알려진 정충 관계)으로 정확히 판정됨을 확인. JS 에러 없음.
결과: ✅ 완료
