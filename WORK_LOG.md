## [2026-07-09 13:50] 뉴스 A/N 단축키 이동 흐름과 내림차순 리스트 인덱스 정합성 최종 교정

**LOG_ID: 20260709_1350**
목표: BBS 뉴스 목록의 데이터 구조(0번 인덱스가 최신 기사, 14번 인덱스가 과거 기사)를 고려하여, 단축키 `A`와 `N` 탐색 시 기사 번호가 거꾸로 역행하거나 한쪽으로만 증가하는 논리 결함을 완전히 바로잡는다.
변경 파일: `public/js/core/commandRouterService.js`
수행 작업:
1) [진단 및 분석] 
   - BBS 뉴스 피드 목록 배열(`articles`)은 최신 기사일수록 앞쪽 인덱스(0번)에, 오래된 기사일수록 뒤쪽 인덱스(14번)에 배치되는 내림차순 정렬 구조이다.
   - 따라서, `A` (이전 기사 = 과거글 = 기사 번호가 감소해야 함)를 누를 때는 인덱스를 **증가(`skipIdx++`)**시켜야 하고, `N` (다음 기사 = 최신글 = 기사 번호가 증가해야 함)을 누를 때는 인덱스를 **감소(`skipIdx--`)**시켜야 한다.
   - 이전 커밋에서 이 목록의 내부 인덱스-대소 관계 흐름을 정반대로 뒤집어 놓아, A와 N 모두 기사 번호가 증가하는 비정상 역행 현상이 발생했다.
2) [수정] 
   - `commandRouterService.js` 의 `A` 단축키는 인덱스 증가 방향(`skipIdx = currentIndex + 1; skipIdx++`)으로, `N` 단축키는 인덱스 감소 방향(`skipIdx = currentIndex - 1; skipIdx--`)으로 수정하여 올바른 번호 증감 흐름을 확보했다.
   - 아울러 `currentIndex === -1` 예외 가드 및 내부 `catch` 백업 루프에서도 `A` 일 때 확실히 번호가 차감(`currentNoNum - 1`, `- 2`)되고, `N` 일 때 확실히 번호가 가산(`currentNoNum + 1`, `+ 2`)되도록 분기를 동기화했다.
3) [검증] `npm test` 유닛 테스트 전체 통과 완료.
실행: `npm test`
기대: 단축키 `A`를 누르면 과거 기사로(번호 감소), `N`을 누르면 최신 기사로(번호 증가) 정확하게 흐르며, 스킵 시에도 방향 꼬임이 발생하지 않는다.
결과: ✅ 완료
