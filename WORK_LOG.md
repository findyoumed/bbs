## [2026-07-09 13:60] 뉴스 A/N 단축키의 BBS 정의(N=번호 감소, A=번호 증가) 및 내림차순 맵핑 최종 복원

**LOG_ID: 20260709_1360**
목표: 사용자 BBS 고유의 원래 스펙(N 키는 시간상 다음글=더 오래된 기사이므로 기사 번호 감소, A 키는 시간상 이전글=더 최신 기사이므로 기사 번호 증가)에 맞추어 단축키 방향을 완전 일치시킨다.
변경 파일: `public/js/core/commandRouterService.js`
수행 작업:
1) [진단 및 분석] 
   - 게시판의 전통적인 목록 뷰는 위에서 아래로 갈수록 글 번호가 작아지는 구조이다.
   - 따라서, 사용자 정의 스펙 상:
     - `N` (Next/다음글): 더 오래된 과거 글을 보아야 하므로 기사 번호 **감소** (배열 인덱스는 뒤로 가야 하므로 `skipIdx++`)
     - `A` (Prev/이전글): 더 최신 글을 보아야 하므로 기사 번호 **증가** (배열 인덱스는 앞으로 돌아가야 하므로 `skipIdx--`)
   - 이를 임의로 역행하여 수정해 두었던 혼선을 정확히 정리하여 되돌렸다.
2) [수정] 
   - `commandRouterService.js` 내 `A` 단축키 동작을 인덱스 감소(`skipIdx--`), `N` 단축키 동작을 인덱스 증가(`skipIdx++`)로 원복시켰다.
   - 경계 예외 가드(`currentIndex === -1`)에서도 기사 번호 가감을 일치하여 `A ➡️ currentNoNum + 1`, `N ➡️ currentNoNum - 1`로 교정했다.
3) [검증] `npm test` 유닛 테스트 정상 통과 확인.
실행: `npm test`
기대: 단축키 `N`을 누르면 오래된 과거 기사(번호 감소), `A`를 누르면 최신 기사(번호 증가) 방향으로 원활히 이동한다.
결과: ✅ 완료
