    ...
    if (s === 'news-view') {
      ...
      if (cmd === 'A' || cmd === 'N') {
        if (currentIndex === -1) {
          ...
          if (currentNoNum > 0) {
            // A(이전글)는 기사 번호가 커지는 방향(+1), N(다음글)은 기사 번호가 작아지는 방향(-1)
      ...
      if (cmd === 'A') {
        // [LOG_ID: 20260709_1360] A(이전글)는 기사 번호가 커지는 방향(인덱스 감소 방향)으로 되돌아간다.
      ...
      if (cmd === 'N') {
        // [LOG_ID: 20260709_1360] N(다음글)은 기사 번호가 작아지는 방향(인덱스 증가 방향)으로 전진한다.
        let skipIdx = currentIndex + 1;
