...
export function createAmusementScreens(deps) {
  ...
  async function showBiorhythm(fromHistory = false) { state.screen = 'bio-input'; state.serviceData = { kind: 'biorhythm' }; if (!fromHistory) updateURL(); await render(buildBiorhythmIntroAnsi(), 'amusementInput', '▶ 생년월일을 8자리로 입력해 주십시오 (예: 19900101) : '); inlineMount('bio-prompt-host', 'game-prompt-host'); }
  ...
  async function showFortune(fromHistory = false) { state.screen = 'fortune-input'; state.serviceData = { kind: 'fortune' }; if (!fromHistory) updateURL(); await render(buildFortuneIntroAnsi(), 'amusementInput', '태어난 연도 입력 (예: 1990) >> '); inlineMount('fortune-prompt-host', 'game-prompt-host'); }
  async function showFortuneResult(input, fromHistory = false) { const year = typeof input === 'number' ? input : validYear(input); if (!year) { setHint('태어난 연도 4자리를 입력하세요. 예) 1990'); return false; } if (typeof restorePromptRow === 'function') { restorePromptRow(); } state.screen = 'fortune-result'; state.serviceData = { kind: 'fortune', birthYear: year }; if (!fromHistory) updateURL(); await render(buildFortuneAnsi(year), 'amusementView', '선택 >> '); return true; }
  ...
  async function showRetroArt(fromHistory = false) { state.screen = 'retro-list'; state.serviceData = { kind: 'retro-art' }; if (!fromHistory) updateURL(); const rendered = await render(buildRetroArtListAnsi(), 'none'); inlineMount('retro-prompt-host', 'game-prompt-host'); if (rendered && rendered.screenNode) { renderRetroArtListHotspots(rendered.screenNode, DOOR_ART, 3); } }
