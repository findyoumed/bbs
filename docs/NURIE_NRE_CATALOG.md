...
- `.NRE` 원본은 일반적인 UTF-8 `ESC[` 스트림이 아니다. Nurie 파일은 CP949/Johab 계열 한글 바이트와 `@[` sentinel(`@[20;1f`, `@[=0F` 등)을 사용하므로, 브라우저 렌더러에 직접 로드하지 않고 참고용으로만 보관한다.
- 브라우저 렌더러(`public/js/core/ansiRenderUtils.js`)의 공통 CSI 지원을 확인하려면 파일의 `@[` sentinel을 `ESC[`로 변환하고, 한글 인코딩은 별도로 복원한 테스트 입력을 사용해야 한다.
- 지원 범위에는 커서 위치·상대 이동(`H/f`, `A/B/C/D`, `E/F/G`, `d`), 화면·행 지우기(`J/K`), 문자·행 삽입/삭제(`@/P/L/M`), 스크롤 영역·스크롤(`r/S/T`), 커서 저장·복원(`s/u`), 색상(`m`)이 포함된다.
- `O`, `=...`, `=b`, `>...`, `g/p/z` 등 Nurie 전용 그래픽·파일 확장은 웹 서비스 renderer의 지원 범위가 아니다. Nurie 전용 파일 포맷, 모뎀 통신, DOS 화면 버퍼, 사운드·키보드 드라이버는 웹 서비스 범위가 아니므로 이 카탈로그의 참고 대상에서 제외한다.
