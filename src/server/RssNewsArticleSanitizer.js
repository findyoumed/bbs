...
// isShortStandaloneAuthorLine은 RssNewsArticleParserScoring.js와 복제 상태였다가 어긋나
// 있던 것을 RssNewsArticleParserText.js로 통합했다(이 파일 쪽 정규식이 올바른 원본).
...
function stripKnownArticleBoilerplateLines(value) {
  ...
  const boilerplatePatterns = [
    ...
    /^AD$/i,
    /^이\s*시각\s*관심정보$/i,
    // [LOG_ID: 20260709_1230] 포털 뉴스 본문에 유입되는 "이미지 확대하기", "사진 확대하기" 등의 레이아웃 텍스트 제거
    /^(?:사진|이미지)\s*(?:확대|확대하기|확대보기|크게보기|보기|저장)$/i,
...
function isScriptCodeDumping(value) {
  ...
  const absoluteCodePatterns = [
    ...
    /displayMetaTag\(/i,
    /modPlayNewsVideo\(/i,
    /viewLayerRemove\.jsp/i,
    /function\s*\(\s*result/i,
    /rptHeader\s*\+=/i
  ];

  for (const pattern of absoluteCodePatterns) {
    if (pattern.test(source)) {
      return true;
    }
  }

  // 2. 라인 단위 분석: 대입 연산자(+=, =), 세미콜론(;), 중괄호({, }), jQuery($) 등으로 이루어진 영문 코드 라인이 본문 라인 중 3개 이상 나타나는 경우
  const lines = source.split('\n').map(l => l.trim()).filter(Boolean);
  let codeLineCount = 0;
  for (const line of lines) {
    if (
      /^[a-zA-Z0-9_$]+\s*\+?=\s*['"a-zA-Z0-9_$]/i.test(line) ||
      /^[a-zA-Z0-9_$]+\(.*?\);?$/i.test(line) ||
      /^(?:if|for|while|switch|catch|return)\b/i.test(line) ||
      /^\$\./.test(line) ||
      /^[a-zA-Z0-9_$]+\s*:\s*['"a-zA-Z0-9_$]/i.test(line)
    ) {
      codeLineCount++;
