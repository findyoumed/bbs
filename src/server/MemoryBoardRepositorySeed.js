...
function seedRoot(repo, boardId, title, content, userId, nickName) {
  ...
  const post = {
    ...
    title,
    content,
    hit: Math.floor((repo.nextPostId * 7) % 91),
    recommend: Math.floor((repo.nextPostId * 3) % 11),
    createdAt: now,
    updatedAt: now
  };
...
function seedReply(repo, boardId, parentPostId, title, content, userId, nickName) {
  ...
  const replyPost = {
    ...
    title,
    content,
    hit: Math.floor((repo.nextPostId * 5) % 57),
    recommend: Math.floor((repo.nextPostId * 2) % 7),
    createdAt: now,
    updatedAt: now
  };
