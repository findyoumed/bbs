...
function looksLikeStructuredTextNoise(field, text, nodeType = '') {
  ...
  // [LOG_ID: 20260712_0140] 구글 버튼 라벨 신표기('펼침', '구글 선호 매체 등록') 추가.
  if (text.length < 160 && /(공유하기|구독하기|좋아요|싫어요|댓글|기사 공유|글자크기|사진\s*확대|이미지\s*확대|원문\s*보기|본문\s*바로가기|카카오톡|페이스북(?:\s*메신저)?|URL\s*복사|펼치기\/접기|^펼침$|요약|구글\s*(?:검색\s*)?선호\s*매체(?:로\s*추가|\s*등록))/.test(text)) {
    return true;
  }
